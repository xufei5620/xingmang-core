...
expect_success "默认 github remote 执行一次镜像" env REAL_GIT="$real_git" D0C_TRACE="$trace" \
  "$mirror_script" --repo "$success_repo" --git-bin "$fake_git"
...
expect_success "dry-run 不执行 push" env REAL_GIT="$real_git" D0C_TRACE="$dry_trace" \
  "$mirror_script" --repo "$dry_repo" --git-bin "$fake_git" --dry-run
...
expect_failure "push 失败时返回非零" env REAL_GIT="$real_git" D0C_TRACE="$fail_trace" D0C_PUSH_FAIL=1 \
  "$mirror_script" --repo "$fail_repo" --git-bin "$fake_git"
...
expect_failure "拒绝非 github remote 名" env REAL_GIT="$real_git" D0C_TRACE="$tmp/blocked.trace" \
  "$mirror_script" --repo "$blocked_repo" --git-bin "$fake_git" --remote origin
...
expect_failure "含凭据的 remote URL 被拒绝" env REAL_GIT="$real_git" D0C_TRACE="$secret_trace" \
  "$mirror_script" --repo "$secret_repo" --git-bin "$fake_git" --dry-run
