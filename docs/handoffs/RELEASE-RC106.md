...
负责人 04:0x 说「那现在发吧」（L1→L2 与 OPS-TRUTH 两条工作流当时仍在跑，有意不等）。

| 步骤 | 开始 | 耗时 | 结果 |
|---|---|---|---|
| 传输 641MB×1 + 3.9MB×2 + 服务器侧校验和 | 04:06:34 | 154s（scp 148s） | 3/3 OK |
| 装镜像 / imageId 比对 / 服务器自验签名 / 66 项校验 | 04:09:47 | ~62s | 9/9 相符；签名 Good；66/66 OK |
| 展开源码 + 环境文件 | 04:10:49 | 1s | `7efece3e`；仅改 `INVOICE_IMAGE_TAG=0.1.0-rc106` |
| 签名备份（停服务，备份脚本自己拉起） | 04:11:26 | 244s | `invoice-20260909T041126Z`，验签 Good；暂存私钥已 shred；结束时 readyz 200 |
| roll-forward（负责人执行） | ~04:18 | ~2m | 步骤 0–6 全绿，`ROLL FORWARD PASS tag=0.1.0-rc106 containers=18 healthz=200 readyz=200` |
| 验证 | 04:21 | — | 18 个容器全部 rc106；readyz 200；API 三分钟内 0 条错误 |

阶段 2 脚本第一次在「核对 manifest 提交」处停下：我写的是 `.gitHead`，manifest 里是
`.source.gitHead`——脚本自己的假设错了，manifest 没错；改成核对 `.source` 段后继续。

部署记录：`/root/invoice-system/deployment-records/rc106-deploy-20260909T040634Z/`。
未做：用户 34 / 12 的用户端目视验收由负责人本人登录确认（我没有他们的会话）。
后续：L1（死信只冻本账号）作为 RC107，基线改为本次生产提交 `7efece3e`。
