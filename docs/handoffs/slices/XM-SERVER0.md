...
### 后端：迁移 000022 + 新包 `internal/platform/server`
...
- `db/migrations/000022_server_registry.{up,down}.sql`（新增）
...
- **真库集成测试**（`internal/platform/server/store_integration_test.go`
  ...
  依次灌 000001~000022 全部正向迁移（含本片新迁移，验证了它在真实
  schema 上能干净应用），建一次性 `xm_test_server0` 角色并
