...
**八、回调联调（XM-CARD4，2026-09-05）：真因是我自己的一行代码。**

生产实测 Infini 一直在发回调（源 IP 16.162.113.86），DNS、nginx、路由、密钥
文件读取全通，只有验签不过。我加了两轮诊断，都把方向指向「密钥值本身不对」，
产品负责人因此重填了三次一个**本来就正确**的密钥。

真因：回调适配器取密钥用了 `SecretValue.String()`，它恒定返回 `"[REDACTED]"`
——那是这个类型防止密钥误入日志的设计。Infini 客户端用的是 `Reveal()`，
我在新写的适配器里用错了。拿一个对所有密钥都相同的常量算 HMAC，签名当然
永远不匹配；那个遮蔽串又不是 base64，于是诊断"如实"地报告密钥不是 base64。

定位靠的不是诊断，是在容器里**数字符**：44 字节、3 个下划线、1 个等号、
base64url 解出 32 字节退出码 0——值是好的，问题只能在取值那一步。

教训写成两条：
- **诊断说"输入不对"时，先独立验证输入，再把人支去改输入。** 我的诊断
  建立在"secret 变量里是真值"的假设上，而假设错了，诊断越精确越误导。
- `SecretValue` 取真值只能用 `Reveal()`。补了测试钉住行为（取出的值不含
  遮蔽标记），改代码前那条测试确实失败，报 `取到的密钥 = "[REDACTED]"`。

修复后第一次真实回调：received 21:14:25.70 → processed 21:14:26.57，attempts=1。
同时定案：冻结态为 `suspend`；信封形状与网页文档一致；签名编码为 hex；
`alias` 在回调里原样回显。
