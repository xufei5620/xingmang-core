...
配置是 `XM_CARDS_<ID>_WITHDRAW_LIMIT_PER_OPERATION` / `_PER_DAY`，**不做
默认值、不回落到卡片额度**。两个都留空 = 该账号不能提现（fail closed），
这是允许的状态：不用提现的部署不该被逼着编两个数字。只配一半、或填
`unlimited`，都在**启动期**拒绝——半配的行为虽安全却表现成「配了却用不了」，
人会以为是 bug 去翻代码。
...
1. **授予 `fund-operator` 角色**给该有提现权限的人。不授予的话提现区会是
   ...
   注册不按配置决定有无。
2. **配提现额度**：`XM_CARDS_<ID>_WITHDRAW_LIMIT_PER_OPERATION` 与
   `_PER_DAY`，真数字，不能是 `unlimited`。不配则该账号不能提现。
