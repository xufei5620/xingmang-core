...
IN_PROGRESS · catalog/journal PostgreSQL runtime implemented; MinIO adapter pending

This branch starts from the exact accepted release tip and records the approved AUD2
input digest below. Both input and generated-artifact approvals are recorded; remaining
work is the provider adapter and database-backed runtime hardening. Shared-stack execution
and production access remain prohibited throughout this slice.
...
- Not done and not claimed: shared/staging application of `000018`, provisioning roles,
  ...
  recorded above; generated bytes are approved but not deployed to the shared stack.
...
- Live MinIO qualification remains pending the one-time disposable CredentialRef
  `secret://archive/minio-qualification`; no secret value is recorded here.
