...
- **时间：** 实现开始 2026-09-08T16:14Z，结束 2026-09-08T16:52Z（约 38 分钟，
  含 15 次变异验证）。
...
- 新增一条**跨文件 grep**：四个消费方的代码里（剥掉注释后）不得再出现
  `"次数"` / `命中 N 次` 这类旧措辞。
...
| `pnpm --filter admin-web run test` | ✅ 143 文件 / 2169 用例 |
| `pnpm --filter ui-admin run test` | ✅ 17 文件 / 262 用例（未改该包，作对照跑） |
...
5. **合并行的 `×N` 与截断提示是同一个事实的两种粒度。** 措辞已分工，但它们在同一
   屏上，将来改文案时要一起看。
6. 所有关于生产现状的数字（288、669、2922）都转引自
