...
**这份规范面向平台内部的功能，不面向平台外的独立服务**（产品负责人
2026-09-06 定）。平台是模块化单体：以后要用接码的功能就在这个进程里，它直接调
领域服务或者走同一套 Action，**不需要另外一套机器身份**。

平台外的独立服务通过 HTTP 打进来才需要「非人」的身份，那件事现在**刻意不做**：
现有的登录系统（Keycloak）后续要弃用，在一个要被换掉的东西上搭机器身份，等换
的时候要拆两遍。等新的登录体系落定，再按那时的样子接。

在那之前，生产上的调用方只有人。开发与预发想模拟一个「非人」的调用方联调，可以
用 dev-header 解析器造（见第 2 节），生产不行——那个解析器在生产会直接拒绝启动。
...
- 平台内部的功能：直接调领域服务，或者以**当前调用者的身份**走 Action，不用
  单独申请身份。
- 需要「非人」身份时（今天只有开发 / 预发的联调）类型必须是 `SERVICE`。
  `AI` 与 `SERVER_AGENT` **不允许**：产品负责人 2026-09-06 定，消费者是平台里的
  功能通过接口接入，AI 会话自己要号不在计划内。
