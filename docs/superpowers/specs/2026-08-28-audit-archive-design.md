...
已批准输入。人类批准 exact base SHA + input digest 后才运行钉版本 sqlc；随后把全部
generated diff 冻结为独立 `generated_artifact_digest`，由 migration owner 做生成制品
review，并再次 **STOP**，获批前不得继续 provider/runtime 实现。任一 input byte、生成器
版本、`sqlc.yaml`、generated byte 或 base 变化都会使对应批准失效。只批准
`archive_segment`，或只有 input approval 而没有 generated-artifact approval，均不足以开工。
...
1. AUD1 开工批准；
2. AUD2 的 migration input digest、sqlc generated-artifact digest、对象供应商、依赖与部署配置批准；
3. AUD4 的 HTTP 契约批准；
