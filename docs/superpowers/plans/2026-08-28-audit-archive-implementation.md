...
Draft the exact numbered up/down files and audit queries. `archive_segment` contains only rows
covered by checkpoint/recovery generation；the three receipt tables implement the
pre-Put durable journal described above. All four tables, constraints/rules, journal
queries, role grants and generated sqlc bytes are inside the same approval digest. Record
...
Dry-run still requires a signed purpose=restore envelope, completes the entire
...
same transaction re-check the bound Kill Switch before opening the write phase and again
...
checks, re-read/reverify the same Kill Switch generation/hash immediately before COMMIT,
...
| PLO/Kill Switch | dry-run candidate cannot mint approval；archive/verify/scrub/restore accept signed approval and emit separate signed result；bound Kill Switch rechecked at every destructive/restricted boundary | AUD3/AUD5 candidate/issuer/domain/result/recheck tests |
