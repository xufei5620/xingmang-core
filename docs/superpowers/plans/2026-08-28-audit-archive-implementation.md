...
- 归档、full verify、scrub、恢复都是 Platform Lifecycle Operation；只接受 signed approval envelope、产生独立 signed result；不增加普通 Action、不增加 UI 触发器。
...
type OptionalArtifactRefV1 struct { Kind, BucketID, Key, VersionID, SHA256 string }
...
Also check in literal existing-signature `ChainRootRefV1` leaf bytes and literal
`ManifestV1`、`CheckpointV1`、`RecoveryIndexV1` `unsigned/unsigned_sha256/signature_*`
envelope bytes and their fixed digest list, plus missing/zero
...
`ChainRootRefV1` keeps the existing root signature in the leaf and is not wrapped in a
second generic envelope. Manifest/Checkpoint/RecoveryIndex use only the common field
names `unsigned, unsigned_sha256, signature_algorithm, signature_key_id, signature`;
no encoder/fixture may emit `unsigned_manifest` or `manifest_sha256` as envelope keys.
...
Before adding CLI implementation, write every Step 9 test and run its full two-package
command once. Expected: FAIL because the command surface and exit-code mapping do not exist.
...
- Create: `internal/platform/audit/archive/receipt_journal_integration_test.go`
...
The journal backing is not implementation-defined: the same exact AUD2 migration creates
`audit.archive_operation_intent`, `audit.archive_put_receipt`, and
`audit.archive_terminal_receipt`. They store canonical intent/ObjectVersion/artifact-ref
bytes plus SHA-256/timestamps, use operation/ordinal uniqueness, and have no-update,
no-delete and no-TRUNCATE enforcement. `xm_audit_archive_receipt_reader` gets fixed-column
SELECT only; `xm_audit_archive_receipt_writer` gets fixed-column INSERT only. They use
distinct CredentialRefs and are neither owner nor members of source/catalog roles.
...
operation intent INSERT is append-only and same operation/different bytes conflicts
put receipt is unique by operation+ordinal and same ordinal/different ObjectVersion conflicts
terminal receipt is append-only and cannot be UPDATE/DELETE/TRUNCATE
receipt reader/writer roles pass exact positive and negative SET ROLE probes
```
...
Expected: FAIL because schema/catalog/journal and their role contracts do not exist.
...
$migrationInputPaths = @(
  ...
  'internal/platform/audit/archive/catalog_integration_test.go',
  'internal/platform/audit/archive/receipt_journal_integration_test.go'
)
$migrationInputManifest = $migrationInputPaths | Sort-Object | ForEach-Object {
  "$(git hash-object -- $_)  $_"
}
...
$generatedPaths = Get-ChildItem internal/platform/audit/gen -File | ForEach-Object {
  $_.FullName.Substring((Get-Location).Path.Length + 1).Replace('\', '/')
}
$generatedManifest = $generatedPaths | Sort-Object | ForEach-Object {
  "$(git hash-object -- $_)  $_"
}
$generatedArtifactDigest = (($generatedManifest -join "`n") + "`n" | git hash-object --stdin)
Write-Output "generated_artifact_digest=$generatedArtifactDigest"
```

Expected: sqlc generation succeeds；`sqlc.yaml` is unchanged；the generator version,
generated manifest and digest are recorded. **STOP again** for a separate generated-artifact
review. The migration owner must approve the exact generated bytes before Step 4；a generator,
...
func TestReceiptJournalSurvivesProcessRestartAndReusesExactVersion(t *testing.T)
func TestReceiptJournalRolesAreAppendOnlyAndSeparated(t *testing.T)
```
...
Use an `httptest` protocol fixture for request signing/headers and the approved provider's
disposable WORM-capable integration bucket. The provider test CredentialRef is a mandatory
AUD2 qualification input；if it is absent, invalid or the live test is SKIP, stop before
adding/committing the S3 adapter, SDK dependency or deployment configuration. Protocol-only
fixtures may remain in a prior filesystem/pure-protocol commit but do not qualify a provider.
...
Expected: protocol tests PASS and approved live provider qualification PASS. A missing
CredentialRef or SKIP is `ProviderQualificationMissing` and blocks AUD2 completion.
When the require flag is `1`, the test must FAIL rather than call `t.Skip` if live
qualification cannot run；the PR evidence records provider/SDK protocol and PASS.
...
Run before implementing the commit orchestration:

```powershell
go test ./internal/platform/audit/archive -count=1 -v
```

Expected: FAIL on the listed commit/receipt/CAS boundary tests. Implement the minimum
protocol described below, rerun the same command, and require PASS.

The order is durable PostgreSQL intent -> payload/projection exact-version verify + append result
...
On process restart with PostgreSQL intact, reload the journal and reuse identical bytes,
idempotency tokens and VersionIDs. If PostgreSQL is lost before CAS, uploaded objects remain
uncommitted orphans and the old operation is not guessed/resumed. If loss is after CAS,
RecoveryIndex/checkpoint rebuild catalog; a missing original result receipt is handled only
by a newly approved archive reconciliation run with its own operation/result.
...
git diff --exit-code -- sqlc.yaml
$generatedPaths = Get-ChildItem internal/platform/audit/gen -File | ForEach-Object { $_.FullName.Substring((Get-Location).Path.Length + 1).Replace('\', '/') }
$actualGeneratedManifest = $generatedPaths | Sort-Object | ForEach-Object { "$(git hash-object -- $_)  $_" }
$actualGeneratedDigest = (($actualGeneratedManifest -join "`n") + "`n" | git hash-object --stdin)
if ($actualGeneratedDigest -ne '<approved-generated-artifact-digest>') { throw 'STOP: generated sqlc artifact changed; generated-artifact approval invalid' }
```
...
type PLOCandidateV1 struct {
    ...
    BuildCommit, BinarySHA256, DryRunDigest string
}
type PLOApprovalEnvelopeV1 struct {
    ...
    Nonce, IdempotencyKey, CandidateSHA256, DryRunDigest, KillSwitchSnapshotSHA256 string
}
...
type PLOResultReceiptV1 struct {
    ...
    Checkpoint OptionalArtifactRefV1
    RecoveryGeneration int64
    RecoveryIndexSHA256 string
    VerificationReport VerificationReport
    ScrubReceipt OptionalArtifactRefV1
    ExitCode int
...
type PLOResultWriter interface {
    PutIfAbsent(context.Context, SignedPLOResultReceiptV1) (ArtifactRefV1, error)
}
...
type Service struct {
    ...
    ReceiptReader ReceiptJournalReader
    ReceiptWriter ReceiptJournalWriter
    ...
    ResultWriter PLOResultWriter
}

func (s *Service) Plan(context.Context, TargetPolicyV1) (ArchivePlan, PLOCandidateV1, []byte, error)
func (s *Service) Archive(context.Context, SignedPLOApprovalEnvelopeV1) (ArchiveResult, error)
func (s *Service) Verify(context.Context, SignedPLOApprovalEnvelopeV1) (VerifyResult, error)
func (s *Service) Scrub(context.Context, SignedPLOApprovalEnvelopeV1) (VerifyResult, error)
```
...
func TestPlanCandidateContainsNoApprovalApproverValidityNonceOrKillSwitchFacts(t *testing.T)
func TestPlanCannotConstructOrSignApprovalEnvelope(t *testing.T)
...
func TestArchiveContinuationUsesTrustedTerminalLocatorAndFirstPrevHash(t *testing.T)
func TestArchiveContinuationRejectsCatalogOnlyOrRangeVerifyAsPredecessor(t *testing.T)
...
func TestVerifyAndScrubRequireSignedEnvelopeAndRecheckKillSwitchBeforeEveryRestrictedGet(t *testing.T)
...
`PLOCandidateV1` canonical bytes + digest. Candidate contains no approval/change ID,
approvers/times, validity, nonce/idempotency key, Kill Switch hash, result or exit code.
It does not sign approval, create root, upload, CAS or write DB. An external approval issuer
combines the candidate digest with two actual eligible HUMAN approval records and the current
signed Kill Switch snapshot, freezes validity/nonce/idempotency, constructs the complete
`PLOApprovalEnvelopeV1`, and signs it. Runtime accepts only that signed artifact；bare
candidate/digest/approval ID and range/bucket/KMS/target CLI overrides are rejected.
...
For `from>1`, obtain `previous_manifest` only from the verified RecoveryIndex/checkpoint
terminal locator, fetch/verify that exact manifest, require `from=previous.to+1` and
`first.prev_hash=previous.last_event_hash`. Catalog latest and existing
`audit.Store.VerifyChain(from,to)` cannot establish this cross-segment boundary.
...
Run before implementing these fault paths:

```powershell
go test ./internal/platform/audit/archive -count=1 -v
```

Expected: FAIL on every newly listed crash/CAS case. Add the minimum convergence logic,
rerun the identical command, and require PASS before continuing.
...
audit-archive verify --envelope <signed-approved-verify-envelope-path>
audit-archive scrub --envelope <signed-approved-scrub-envelope-path>
```

Plan emits only `PLOCandidateV1`; the independent approval system emits the signed artifact.
Archive/verify/scrub accept only their matching signed envelope and always persist a separately
signed PLO result receipt. Recovery locator, bucket/prefix/credential refs come from signed
envelope + approved config, not flags.
...
RecoveryIndexCASWriter fixed CAS, ReceiptJournalReader fixed SELECT,
ReceiptJournalWriter fixed-column append, ScrubReceipt/Head writer and
...
$coverageJSON = [regex]::Match($specText, '(?s)建议在现有响应顶层新增：\s*```json\s*(?<json>.*?)\s*```').Groups['json'].Value
if ([string]::IsNullOrWhiteSpace($coverageJSON)) { throw 'coverage JSON block not found' }
...
type Restorer struct {
    ...
    ResultKeyring   Keyring
    KillSwitchKeyring Keyring
    ResultSigner    PurposeSigner
    ResultWriter    PLOResultWriter
    ReceiptReader   ReceiptJournalReader
    ReceiptWriter   ReceiptJournalWriter
    Target         *pgxpool.Pool
...
func (r *Restorer) Restore(context.Context, RestoreRequest) (RestoreResult, error)
```
...
func TestRestoreRejectsKillSwitchWrongPurposeProtocolSignatureOrKey(t *testing.T)
func TestRestoreRechecksBoundKillSwitchBeforeTransactionFirstInsertAndCommit(t *testing.T)
func TestRestorePersistsSeparateSignedResultReceiptForSuccessAndFailure(t *testing.T)
func TestRestoreResultSignerPurposeAndResultJournalAreRequired(t *testing.T)
...
Dry-run still requires a signed purpose=restore envelope and an explicitly injected
`KillSwitchKeyring`. It verifies the bound snapshot's purpose/protocol/signature/key before
any Restricted Get, completes the entire
manifest/object/root/chain validation, checks target identity/emptiness, persists a
separate signed PLO result receipt, and prints its report projection without beginning
an insert transaction.

- [ ] **Step 3: Implement exact-field importer**

Before implementation, write every Step 4 mixed-version/transaction/result test and run:

```powershell
go test ./internal/platform/audit/archive -count=1 -v
```

Expected: FAIL on all newly listed restore round-trip cases. Preserve this RED output,
then implement the importer below.

Use a dedicated, versioned importer with fixed INSERT columns. It does not invoke
`Store.Append`. After initial verification through `KillSwitchKeyring`, re-read the fixed
locator and verify the same bound purpose/protocol/signature/key + generation/hash before
opening the target transaction；inside that same transaction re-read and reverify it again
before the first INSERT, re-check isolation/emptiness, insert all rows, run a tx-bound
`VerifyChain(1, root.ToSequence)`, canonical counts/row count/tip/root and Query parity
checks, then re-read/reverify the same signed Kill Switch purpose/protocol/signature/key +
generation/hash a third time immediately before COMMIT,
then COMMIT. Any Kill Switch or verify failure ROLLBACKS all rows；no pool-based verifier may
...
func TestRestoreProducesSignedResultBoundToApprovalAndCheckpoint(t *testing.T)
...
Expected: PASS, including `TestRestorePostInsert*`, `TestRestoreProduces*` and
`TestRestoreRejects*`; no production/staging database is accepted as target.
...
Plan emits an unsigned `PLOCandidateV1`; an independent approval issuer constructs the
signed purpose=restore envelope. Source fixed RecoveryIndex locator and target fingerprint come through signed envelope
...
PII, DSN or key material. The drill is complete only when the signed restore result
receipt is durably stored and its digest appears in evidence；an unsigned RestoreReport
...
Run before registering any scheduler/configuration:

```powershell
go test ./internal/platform/jobs ./cmd/platform-worker -count=1 -v
```

Expected: FAIL on every newly listed `TestAuditArchive*` case. Only after that RED evidence
may the minimum registration be added；rerun the identical command and require PASS.
...
| frozen signed wire | `ChainRootRefV1` existing-signature leaf exception + one common envelope for Manifest/Checkpoint/RecoveryIndex | AUD1 literal bytes/SHA, unique field-name, missing/null/order/time and cross-domain goldens |
...
| ambiguous Put | append-only PG intent/put/terminal journal before Put + mandatory approved-provider `RecoverPutResult`; missing/SKIP qualification is STOP | AUD2 restart/response-loss/live-provider/role/receipt fault injection |
| precise capabilities | split Source snapshot/Root/Catalog/Object/Recovery/Receipt reader+writer/Scrub/Access interfaces, credentials and `RecoverySigner` | AUD3 method-shape plus DB/IAM positive/negative integration evidence |
| PLO/Kill Switch | dry-run candidate cannot mint approval；archive/verify/scrub/restore accept signed approval and emit separate signed result；Service/Restorer explicitly inject independent `KillSwitchKeyring` and recheck the bound snapshot at every destructive/restricted boundary | AUD3/AUD5 candidate/issuer/domain/result/wrong-purpose-protocol-signature-key/recheck tests |
| first boundary | AUD1 local export starts only at sequence 1；continuation uses trusted terminal locator + first prev hash, never range VerifyChain/catalog | AUD1 CLI rejection；AUD2 inverse walk；AUD3 writer-boundary tests |
