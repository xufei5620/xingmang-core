...
**Architecture:** RC100 is a source-agent, api and console roll-forward from RC99, the release in production. **No migration, no evaluator change.** With the cutover variables set to the manifests' own values the agents behave exactly as today; the approved pin stays at `0.1.179` through this roll-forward, and the pin bump to `0.2.1` is a separate CAS afterwards, by the runbook's procedure. Rollback is the RC99 release directory.
...
- [ ] After the canary, as its own step with its own record: the Sub2API pin bump `0.1.179 → 0.2.1` by the runbook's source-upgrade CAS (bridge contract re-audited against v0.2.1: `usage_logs` and `users` gained columns only), then a second 30-minute watch of the five Sub2API agents.
