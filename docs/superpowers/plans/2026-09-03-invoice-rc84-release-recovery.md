...
- Canary: 30-minute readiness watch from 17:39Z; the product owner's assertion canary is the remaining step, and its evidence is the first `auth.console_assertion.exchanged` row in invoice `audit_events` (all three previous canaries produced only `auth.console_assertion.rejected`).
