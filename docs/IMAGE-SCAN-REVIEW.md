...
> They are not renamed or rewritten as RC92 evidence.  The current RC92 section
...
## Current RC92 remediation and review status (source/static only)

RC92 is an urgent remediation slice, not an approved release.  The RC48
...
amended, or used as RC92 evidence. RC92 requires a new exact tag, nine newly
...
as read-only and never resume, reuse, amend, rename, or represent it as RC92
evidence.
...
resume, amend, rename, reuse, or represent it as RC92 evidence.
...
### Actual RC92 source changes awaiting evidence
...
- Keycloak refreshes its base to
  ...
  from the existing hardening; it is required evidence, not an RC92 source
  remediation.

The intended RC92 inventory is nine manifest-bound images under one exact,
...
must fail closed.  This inventory is a source contract until the RC92 release
gate produces and independently verifies the manifest and its IDs.

### RC92 exception policy

PostgreSQL has **no RC92 exception**.  Its policy is zero findings: every old
...
The sole permitted RC92 exception is the retained raw Keycloak finding with
this exact tuple:
...
`release-candidate` tags are not RC92 references and do not authorize current
...
changed. It is not permitted for RC92, whose PostgreSQL policy is zero findings
...
production OIDC/MFA/logout canary. The current RC92 path is defined only by the
exact tuple and refreshed base digest in the bounded RC92 section at the top of
this document.
