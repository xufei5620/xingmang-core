...
> They are not renamed or rewritten as RC57 evidence.  The current RC57 section
...
## Current RC57 remediation and review status (source/static only)

RC57 is an urgent remediation slice, not an approved release.  The RC48
...
amended, or used as RC57 evidence. RC57 requires a new exact tag, nine newly
...
as read-only and never resume, reuse, amend, rename, or represent it as RC57
evidence.
...
resume, amend, rename, reuse, or represent it as RC57 evidence.
...
### Actual RC57 source changes awaiting evidence
...
- Keycloak refreshes its base to
  ...
  from the existing hardening; it is required evidence, not an RC57 source
  remediation.

The intended RC57 inventory is nine manifest-bound images under one exact,
...
must fail closed.  This inventory is a source contract until the RC57 release
gate produces and independently verifies the manifest and its IDs.

### RC57 exception policy

PostgreSQL has **no RC57 exception**.  Its policy is zero findings: every old
...
The sole permitted RC57 exception is the retained raw Keycloak finding with
this exact tuple:
...
`release-candidate` tags are not RC57 references and do not authorize current
...
changed. It is not permitted for RC57, whose PostgreSQL policy is zero findings
...
production OIDC/MFA/logout canary. The current RC57 path is defined only by the
exact tuple and refreshed base digest in the bounded RC57 section at the top of
this document.
