...
> They are not renamed or rewritten as RC70 evidence.  The current RC70 section
...
## Current RC70 remediation and review status (source/static only)

RC70 is an urgent remediation slice, not an approved release.  The RC48
...
amended, or used as RC70 evidence. RC70 requires a new exact tag, nine newly
...
as read-only and never resume, reuse, amend, rename, or represent it as RC70
evidence.
...
resume, amend, rename, reuse, or represent it as RC70 evidence.
...
### Actual RC70 source changes awaiting evidence
...
- Keycloak refreshes its base to
  ...
  from the existing hardening; it is required evidence, not an RC70 source
  remediation.

The intended RC70 inventory is nine manifest-bound images under one exact,
...
must fail closed.  This inventory is a source contract until the RC70 release
gate produces and independently verifies the manifest and its IDs.

### RC70 exception policy

PostgreSQL has **no RC70 exception**.  Its policy is zero findings: every old
...
The sole permitted RC70 exception is the retained raw Keycloak finding with
this exact tuple:
...
`release-candidate` tags are not RC70 references and do not authorize current
...
changed. It is not permitted for RC70, whose PostgreSQL policy is zero findings
...
production OIDC/MFA/logout canary. The current RC70 path is defined only by the
exact tuple and refreshed base digest in the bounded RC70 section at the top of
this document.
