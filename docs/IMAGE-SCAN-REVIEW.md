...
> They are not renamed or rewritten as RC81 evidence.  The current RC81 section
...
## Current RC81 remediation and review status (source/static only)

RC81 is an urgent remediation slice, not an approved release.  The RC48
...
amended, or used as RC81 evidence. RC81 requires a new exact tag, nine newly
...
as read-only and never resume, reuse, amend, rename, or represent it as RC81
evidence.
...
resume, amend, rename, reuse, or represent it as RC81 evidence.
...
### Actual RC81 source changes awaiting evidence
...
- Keycloak refreshes its base to
  ...
  from the existing hardening; it is required evidence, not an RC81 source
  remediation.

The intended RC81 inventory is nine manifest-bound images under one exact,
...
must fail closed.  This inventory is a source contract until the RC81 release
gate produces and independently verifies the manifest and its IDs.

### RC81 exception policy

PostgreSQL has **no RC81 exception**.  Its policy is zero findings: every old
...
The sole permitted RC81 exception is the retained raw Keycloak finding with
this exact tuple:
...
`release-candidate` tags are not RC81 references and do not authorize current
...
changed. It is not permitted for RC81, whose PostgreSQL policy is zero findings
...
production OIDC/MFA/logout canary. The current RC81 path is defined only by the
exact tuple and refreshed base digest in the bounded RC81 section at the top of
this document.
