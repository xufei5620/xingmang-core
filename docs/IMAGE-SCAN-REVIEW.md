...
> They are not renamed or rewritten as RC79 evidence.  The current RC79 section
...
## Current RC79 remediation and review status (source/static only)

RC79 is an urgent remediation slice, not an approved release.  The RC48
...
amended, or used as RC79 evidence. RC79 requires a new exact tag, nine newly
...
as read-only and never resume, reuse, amend, rename, or represent it as RC79
evidence.
...
resume, amend, rename, reuse, or represent it as RC79 evidence.
...
### Actual RC79 source changes awaiting evidence
...
- Keycloak refreshes its base to
  ...
  from the existing hardening; it is required evidence, not an RC79 source
  remediation.

The intended RC79 inventory is nine manifest-bound images under one exact,
...
must fail closed.  This inventory is a source contract until the RC79 release
gate produces and independently verifies the manifest and its IDs.

### RC79 exception policy

PostgreSQL has **no RC79 exception**.  Its policy is zero findings: every old
...
The sole permitted RC79 exception is the retained raw Keycloak finding with
this exact tuple:
...
`release-candidate` tags are not RC79 references and do not authorize current
...
changed. It is not permitted for RC79, whose PostgreSQL policy is zero findings
...
production OIDC/MFA/logout canary. The current RC79 path is defined only by the
exact tuple and refreshed base digest in the bounded RC79 section at the top of
this document.
