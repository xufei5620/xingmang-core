...
> They are not renamed or rewritten as RC89 evidence.  The current RC89 section
...
## Current RC89 remediation and review status (source/static only)

RC89 is an urgent remediation slice, not an approved release.  The RC48
...
amended, or used as RC89 evidence. RC89 requires a new exact tag, nine newly
...
as read-only and never resume, reuse, amend, rename, or represent it as RC89
evidence.
...
resume, amend, rename, reuse, or represent it as RC89 evidence.
...
### Actual RC89 source changes awaiting evidence
...
- Keycloak refreshes its base to
  ...
  from the existing hardening; it is required evidence, not an RC89 source
  remediation.

The intended RC89 inventory is nine manifest-bound images under one exact,
...
must fail closed.  This inventory is a source contract until the RC89 release
gate produces and independently verifies the manifest and its IDs.

### RC89 exception policy

PostgreSQL has **no RC89 exception**.  Its policy is zero findings: every old
...
The sole permitted RC89 exception is the retained raw Keycloak finding with
this exact tuple:
...
`release-candidate` tags are not RC89 references and do not authorize current
...
changed. It is not permitted for RC89, whose PostgreSQL policy is zero findings
...
production OIDC/MFA/logout canary. The current RC89 path is defined only by the
exact tuple and refreshed base digest in the bounded RC89 section at the top of
this document.
