...
> They are not renamed or rewritten as RC100 evidence.  The current RC100 section
...
## Current RC100 remediation and review status (source/static only)

RC100 is an urgent remediation slice, not an approved release.  The RC48
...
amended, or used as RC100 evidence. RC100 requires a new exact tag, nine newly
...
as read-only and never resume, reuse, amend, rename, or represent it as RC100
evidence.
...
resume, amend, rename, reuse, or represent it as RC100 evidence.
...
### Actual RC100 source changes awaiting evidence
...
- Keycloak refreshes its base to
  ...
  from the existing hardening; it is required evidence, not an RC100 source
  remediation.

The intended RC100 inventory is nine manifest-bound images under one exact,
...
must fail closed.  This inventory is a source contract until the RC100 release
gate produces and independently verifies the manifest and its IDs.

### RC100 exception policy

PostgreSQL has **no RC100 exception**.  Its policy is zero findings: every old
...
The sole permitted RC100 exception is the retained raw Keycloak finding with
this exact tuple:
...
`release-candidate` tags are not RC100 references and do not authorize current
...
changed. It is not permitted for RC100, whose PostgreSQL policy is zero findings
...
production OIDC/MFA/logout canary. The current RC100 path is defined only by the
exact tuple and refreshed base digest in the bounded RC100 section at the top of
this document.
