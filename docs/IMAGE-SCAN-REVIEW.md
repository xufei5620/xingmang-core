...
> They are not renamed or rewritten as RC87 evidence.  The current RC87 section
...
## Current RC87 remediation and review status (source/static only)

RC87 is an urgent remediation slice, not an approved release.  The RC48
...
amended, or used as RC87 evidence. RC87 requires a new exact tag, nine newly
...
as read-only and never resume, reuse, amend, rename, or represent it as RC87
evidence.
...
resume, amend, rename, reuse, or represent it as RC87 evidence.
...
### Actual RC87 source changes awaiting evidence
...
- Keycloak refreshes its base to
  ...
  from the existing hardening; it is required evidence, not an RC87 source
  remediation.

The intended RC87 inventory is nine manifest-bound images under one exact,
...
must fail closed.  This inventory is a source contract until the RC87 release
gate produces and independently verifies the manifest and its IDs.

### RC87 exception policy

PostgreSQL has **no RC87 exception**.  Its policy is zero findings: every old
...
The sole permitted RC87 exception is the retained raw Keycloak finding with
this exact tuple:
...
`release-candidate` tags are not RC87 references and do not authorize current
...
changed. It is not permitted for RC87, whose PostgreSQL policy is zero findings
...
production OIDC/MFA/logout canary. The current RC87 path is defined only by the
exact tuple and refreshed base digest in the bounded RC87 section at the top of
this document.
