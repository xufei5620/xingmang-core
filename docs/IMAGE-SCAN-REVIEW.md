...
> They are not renamed or rewritten as RC99 evidence.  The current RC99 section
...
## Current RC99 remediation and review status (source/static only)

RC99 is an urgent remediation slice, not an approved release.  The RC48
...
amended, or used as RC99 evidence. RC99 requires a new exact tag, nine newly
...
as read-only and never resume, reuse, amend, rename, or represent it as RC99
evidence.
...
resume, amend, rename, reuse, or represent it as RC99 evidence.
...
### Actual RC99 source changes awaiting evidence
...
- Keycloak refreshes its base to
  ...
  from the existing hardening; it is required evidence, not an RC99 source
  remediation.

The intended RC99 inventory is nine manifest-bound images under one exact,
...
must fail closed.  This inventory is a source contract until the RC99 release
gate produces and independently verifies the manifest and its IDs.

### RC99 exception policy

PostgreSQL has **no RC99 exception**.  Its policy is zero findings: every old
...
The sole permitted RC99 exception is the retained raw Keycloak finding with
this exact tuple:
...
`release-candidate` tags are not RC99 references and do not authorize current
...
changed. It is not permitted for RC99, whose PostgreSQL policy is zero findings
...
production OIDC/MFA/logout canary. The current RC99 path is defined only by the
exact tuple and refreshed base digest in the bounded RC99 section at the top of
this document.
