...
> They are not renamed or rewritten as RC71 evidence.  The current RC71 section
...
## Current RC71 remediation and review status (source/static only)

RC71 is an urgent remediation slice, not an approved release.  The RC48
...
amended, or used as RC71 evidence. RC71 requires a new exact tag, nine newly
...
as read-only and never resume, reuse, amend, rename, or represent it as RC71
evidence.
...
resume, amend, rename, reuse, or represent it as RC71 evidence.
...
### Actual RC71 source changes awaiting evidence
...
- Keycloak refreshes its base to
  ...
  from the existing hardening; it is required evidence, not an RC71 source
  remediation.

The intended RC71 inventory is nine manifest-bound images under one exact,
...
must fail closed.  This inventory is a source contract until the RC71 release
gate produces and independently verifies the manifest and its IDs.

### RC71 exception policy

PostgreSQL has **no RC71 exception**.  Its policy is zero findings: every old
...
The sole permitted RC71 exception is the retained raw Keycloak finding with
this exact tuple:
...
`release-candidate` tags are not RC71 references and do not authorize current
...
changed. It is not permitted for RC71, whose PostgreSQL policy is zero findings
...
production OIDC/MFA/logout canary. The current RC71 path is defined only by the
exact tuple and refreshed base digest in the bounded RC71 section at the top of
this document.
