...
> They are not renamed or rewritten as RC88 evidence.  The current RC88 section
...
## Current RC88 remediation and review status (source/static only)

RC88 is an urgent remediation slice, not an approved release.  The RC48
...
amended, or used as RC88 evidence. RC88 requires a new exact tag, nine newly
...
as read-only and never resume, reuse, amend, rename, or represent it as RC88
evidence.
...
resume, amend, rename, reuse, or represent it as RC88 evidence.
...
### Actual RC88 source changes awaiting evidence
...
- Keycloak refreshes its base to
  ...
  from the existing hardening; it is required evidence, not an RC88 source
  remediation.

The intended RC88 inventory is nine manifest-bound images under one exact,
...
must fail closed.  This inventory is a source contract until the RC88 release
gate produces and independently verifies the manifest and its IDs.

### RC88 exception policy

PostgreSQL has **no RC88 exception**.  Its policy is zero findings: every old
...
The sole permitted RC88 exception is the retained raw Keycloak finding with
this exact tuple:
...
`release-candidate` tags are not RC88 references and do not authorize current
...
changed. It is not permitted for RC88, whose PostgreSQL policy is zero findings
...
production OIDC/MFA/logout canary. The current RC88 path is defined only by the
exact tuple and refreshed base digest in the bounded RC88 section at the top of
this document.
