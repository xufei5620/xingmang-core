...
> They are not renamed or rewritten as RC86 evidence.  The current RC86 section
...
## Current RC86 remediation and review status (source/static only)

RC86 is an urgent remediation slice, not an approved release.  The RC48
...
amended, or used as RC86 evidence. RC86 requires a new exact tag, nine newly
...
as read-only and never resume, reuse, amend, rename, or represent it as RC86
evidence.
...
resume, amend, rename, reuse, or represent it as RC86 evidence.
...
### Actual RC86 source changes awaiting evidence
...
- Keycloak refreshes its base to
  ...
  from the existing hardening; it is required evidence, not an RC86 source
  remediation.

The intended RC86 inventory is nine manifest-bound images under one exact,
...
must fail closed.  This inventory is a source contract until the RC86 release
gate produces and independently verifies the manifest and its IDs.

### RC86 exception policy

PostgreSQL has **no RC86 exception**.  Its policy is zero findings: every old
...
The sole permitted RC86 exception is the retained raw Keycloak finding with
this exact tuple:
...
`release-candidate` tags are not RC86 references and do not authorize current
...
changed. It is not permitted for RC86, whose PostgreSQL policy is zero findings
...
production OIDC/MFA/logout canary. The current RC86 path is defined only by the
exact tuple and refreshed base digest in the bounded RC86 section at the top of
this document.
