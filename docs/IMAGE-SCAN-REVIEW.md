...
> They are not renamed or rewritten as RC83 evidence.  The current RC83 section
...
## Current RC83 remediation and review status (source/static only)

RC83 is an urgent remediation slice, not an approved release.  The RC48
...
amended, or used as RC83 evidence. RC83 requires a new exact tag, nine newly
...
as read-only and never resume, reuse, amend, rename, or represent it as RC83
evidence.
...
resume, amend, rename, reuse, or represent it as RC83 evidence.
...
### Actual RC83 source changes awaiting evidence
...
- Keycloak refreshes its base to
  ...
  from the existing hardening; it is required evidence, not an RC83 source
  remediation.

The intended RC83 inventory is nine manifest-bound images under one exact,
...
must fail closed.  This inventory is a source contract until the RC83 release
gate produces and independently verifies the manifest and its IDs.

### RC83 exception policy

PostgreSQL has **no RC83 exception**.  Its policy is zero findings: every old
...
The sole permitted RC83 exception is the retained raw Keycloak finding with
this exact tuple:
...
`release-candidate` tags are not RC83 references and do not authorize current
...
changed. It is not permitted for RC83, whose PostgreSQL policy is zero findings
...
production OIDC/MFA/logout canary. The current RC83 path is defined only by the
exact tuple and refreshed base digest in the bounded RC83 section at the top of
this document.
