...
> They are not renamed or rewritten as RC67 evidence.  The current RC67 section
...
## Current RC67 remediation and review status (source/static only)

RC67 is an urgent remediation slice, not an approved release.  The RC48
...
amended, or used as RC67 evidence. RC67 requires a new exact tag, nine newly
...
as read-only and never resume, reuse, amend, rename, or represent it as RC67
evidence.
...
resume, amend, rename, reuse, or represent it as RC67 evidence.
...
### Actual RC67 source changes awaiting evidence
...
- Keycloak refreshes its base to
  ...
  from the existing hardening; it is required evidence, not an RC67 source
  remediation.

The intended RC67 inventory is nine manifest-bound images under one exact,
...
must fail closed.  This inventory is a source contract until the RC67 release
gate produces and independently verifies the manifest and its IDs.

### RC67 exception policy

PostgreSQL has **no RC67 exception**.  Its policy is zero findings: every old
...
The sole permitted RC67 exception is the retained raw Keycloak finding with
this exact tuple:
...
`release-candidate` tags are not RC67 references and do not authorize current
...
changed. It is not permitted for RC67, whose PostgreSQL policy is zero findings
...
production OIDC/MFA/logout canary. The current RC67 path is defined only by the
exact tuple and refreshed base digest in the bounded RC67 section at the top of
this document.
