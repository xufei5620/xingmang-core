...
> They are not renamed or rewritten as RC73 evidence.  The current RC73 section
...
## Current RC73 remediation and review status (source/static only)

RC73 is an urgent remediation slice, not an approved release.  The RC48
...
amended, or used as RC73 evidence. RC73 requires a new exact tag, nine newly
...
as read-only and never resume, reuse, amend, rename, or represent it as RC73
evidence.
...
resume, amend, rename, reuse, or represent it as RC73 evidence.
...
### Actual RC73 source changes awaiting evidence
...
- Keycloak refreshes its base to
  ...
  from the existing hardening; it is required evidence, not an RC73 source
  remediation.

The intended RC73 inventory is nine manifest-bound images under one exact,
...
must fail closed.  This inventory is a source contract until the RC73 release
gate produces and independently verifies the manifest and its IDs.

### RC73 exception policy

PostgreSQL has **no RC73 exception**.  Its policy is zero findings: every old
...
The sole permitted RC73 exception is the retained raw Keycloak finding with
this exact tuple:
...
`release-candidate` tags are not RC73 references and do not authorize current
...
changed. It is not permitted for RC73, whose PostgreSQL policy is zero findings
...
production OIDC/MFA/logout canary. The current RC73 path is defined only by the
exact tuple and refreshed base digest in the bounded RC73 section at the top of
this document.
