...
> They are not renamed or rewritten as RC82 evidence.  The current RC82 section
...
## Current RC82 remediation and review status (source/static only)

RC82 is an urgent remediation slice, not an approved release.  The RC48
...
amended, or used as RC82 evidence. RC82 requires a new exact tag, nine newly
...
as read-only and never resume, reuse, amend, rename, or represent it as RC82
evidence.
...
resume, amend, rename, reuse, or represent it as RC82 evidence.
...
### Actual RC82 source changes awaiting evidence
...
- Keycloak refreshes its base to
  ...
  from the existing hardening; it is required evidence, not an RC82 source
  remediation.

The intended RC82 inventory is nine manifest-bound images under one exact,
...
must fail closed.  This inventory is a source contract until the RC82 release
gate produces and independently verifies the manifest and its IDs.

### RC82 exception policy

PostgreSQL has **no RC82 exception**.  Its policy is zero findings: every old
...
The sole permitted RC82 exception is the retained raw Keycloak finding with
this exact tuple:
...
`release-candidate` tags are not RC82 references and do not authorize current
...
changed. It is not permitted for RC82, whose PostgreSQL policy is zero findings
...
production OIDC/MFA/logout canary. The current RC82 path is defined only by the
exact tuple and refreshed base digest in the bounded RC82 section at the top of
this document.
