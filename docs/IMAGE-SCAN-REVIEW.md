...
> They are not renamed or rewritten as RC66 evidence.  The current RC66 section
...
## Current RC66 remediation and review status (source/static only)

RC66 is an urgent remediation slice, not an approved release.  The RC48
...
amended, or used as RC66 evidence. RC66 requires a new exact tag, nine newly
...
as read-only and never resume, reuse, amend, rename, or represent it as RC66
evidence.
...
resume, amend, rename, reuse, or represent it as RC66 evidence.
...
### Actual RC66 source changes awaiting evidence
...
- Keycloak refreshes its base to
  ...
  from the existing hardening; it is required evidence, not an RC66 source
  remediation.

The intended RC66 inventory is nine manifest-bound images under one exact,
...
must fail closed.  This inventory is a source contract until the RC66 release
gate produces and independently verifies the manifest and its IDs.

### RC66 exception policy

PostgreSQL has **no RC66 exception**.  Its policy is zero findings: every old
...
The sole permitted RC66 exception is the retained raw Keycloak finding with
this exact tuple:
...
`release-candidate` tags are not RC66 references and do not authorize current
...
changed. It is not permitted for RC66, whose PostgreSQL policy is zero findings
...
production OIDC/MFA/logout canary. The current RC66 path is defined only by the
exact tuple and refreshed base digest in the bounded RC66 section at the top of
this document.
