...
> They are not renamed or rewritten as RC52 evidence.  The current RC52 section
...
## Current RC52 remediation and review status (source/static only)

RC52 is an urgent remediation slice, not an approved release.  The RC48
...
amended, or used as RC52 evidence. RC52 requires a new exact tag, nine newly
...
as read-only and never resume, reuse, amend, rename, or represent it as RC52
evidence.
...
resume, amend, rename, reuse, or represent it as RC52 evidence.
...
### Actual RC52 source changes awaiting evidence
...
- Keycloak refreshes its base to
  ...
  from the existing hardening; it is required evidence, not an RC52 source
  remediation.

The intended RC52 inventory is nine manifest-bound images under one exact,
...
must fail closed.  This inventory is a source contract until the RC52 release
gate produces and independently verifies the manifest and its IDs.

### RC52 exception policy

PostgreSQL has **no RC52 exception**.  Its policy is zero findings: every old
...
The sole permitted RC52 exception is the retained raw Keycloak finding with
this exact tuple:
...
`release-candidate` tags are not RC52 references and do not authorize current
...
changed. It is not permitted for RC52, whose PostgreSQL policy is zero findings
...
production OIDC/MFA/logout canary. The current RC52 path is defined only by the
exact tuple and refreshed base digest in the bounded RC52 section at the top of
this document.
