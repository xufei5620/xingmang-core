...
> They are not renamed or rewritten as RC65 evidence.  The current RC65 section
...
## Current RC65 remediation and review status (source/static only)

RC65 is an urgent remediation slice, not an approved release.  The RC48
...
amended, or used as RC65 evidence. RC65 requires a new exact tag, nine newly
...
as read-only and never resume, reuse, amend, rename, or represent it as RC65
evidence.
...
resume, amend, rename, reuse, or represent it as RC65 evidence.
...
### Actual RC65 source changes awaiting evidence
...
- Keycloak refreshes its base to
  ...
  from the existing hardening; it is required evidence, not an RC65 source
  remediation.

The intended RC65 inventory is nine manifest-bound images under one exact,
...
must fail closed.  This inventory is a source contract until the RC65 release
gate produces and independently verifies the manifest and its IDs.

### RC65 exception policy

PostgreSQL has **no RC65 exception**.  Its policy is zero findings: every old
...
The sole permitted RC65 exception is the retained raw Keycloak finding with
this exact tuple:
...
`release-candidate` tags are not RC65 references and do not authorize current
...
changed. It is not permitted for RC65, whose PostgreSQL policy is zero findings
...
production OIDC/MFA/logout canary. The current RC65 path is defined only by the
exact tuple and refreshed base digest in the bounded RC65 section at the top of
this document.
