...
> They are not renamed or rewritten as RC95 evidence.  The current RC95 section
...
## Current RC95 remediation and review status (source/static only)

RC95 is an urgent remediation slice, not an approved release.  The RC48
...
amended, or used as RC95 evidence. RC95 requires a new exact tag, nine newly
...
as read-only and never resume, reuse, amend, rename, or represent it as RC95
evidence.
...
resume, amend, rename, reuse, or represent it as RC95 evidence.
...
### Actual RC95 source changes awaiting evidence
...
- Keycloak refreshes its base to
  ...
  from the existing hardening; it is required evidence, not an RC95 source
  remediation.

The intended RC95 inventory is nine manifest-bound images under one exact,
...
must fail closed.  This inventory is a source contract until the RC95 release
gate produces and independently verifies the manifest and its IDs.

### RC95 exception policy

PostgreSQL has **no RC95 exception**.  Its policy is zero findings: every old
...
The sole permitted RC95 exception is the retained raw Keycloak finding with
this exact tuple:
...
`release-candidate` tags are not RC95 references and do not authorize current
...
changed. It is not permitted for RC95, whose PostgreSQL policy is zero findings
...
production OIDC/MFA/logout canary. The current RC95 path is defined only by the
exact tuple and refreshed base digest in the bounded RC95 section at the top of
this document.
