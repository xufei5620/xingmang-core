...
> They are not renamed or rewritten as RC51 evidence.  The current RC51 section
...
## Current RC51 remediation and review status (source/static only)

RC51 is an urgent remediation slice, not an approved release.  The RC48
...
amended, or used as RC51 evidence. RC51 requires a new exact tag, nine newly
...
as read-only and never resume, reuse, amend, rename, or represent it as RC51
evidence.
...
### Actual RC51 source changes awaiting evidence
...
- Keycloak refreshes its base to
  ...
  from the existing hardening; it is required evidence, not an RC51 source
  remediation.

The intended RC51 inventory is nine manifest-bound images under one exact,
...
must fail closed.  This inventory is a source contract until the RC51 release
gate produces and independently verifies the manifest and its IDs.

### RC51 exception policy

PostgreSQL has **no RC51 exception**.  Its policy is zero findings: every old
...
The sole permitted RC51 exception is the retained raw Keycloak finding with
this exact tuple:
...
`release-candidate` tags are not RC51 references and do not authorize current
...
changed. It is not permitted for RC51, whose PostgreSQL policy is zero findings
...
production OIDC/MFA/logout canary. The current RC51 path is defined only by the
exact tuple and refreshed base digest in the bounded RC51 section at the top of
this document.
