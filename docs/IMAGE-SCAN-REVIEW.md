...
> They are not renamed or rewritten as RC77 evidence.  The current RC77 section
...
## Current RC77 remediation and review status (source/static only)

RC77 is an urgent remediation slice, not an approved release.  The RC48
...
amended, or used as RC77 evidence. RC77 requires a new exact tag, nine newly
...
as read-only and never resume, reuse, amend, rename, or represent it as RC77
evidence.
...
resume, amend, rename, reuse, or represent it as RC77 evidence.
...
### Actual RC77 source changes awaiting evidence
...
- Keycloak refreshes its base to
  ...
  from the existing hardening; it is required evidence, not an RC77 source
  remediation.

The intended RC77 inventory is nine manifest-bound images under one exact,
...
must fail closed.  This inventory is a source contract until the RC77 release
gate produces and independently verifies the manifest and its IDs.

### RC77 exception policy

PostgreSQL has **no RC77 exception**.  Its policy is zero findings: every old
...
The sole permitted RC77 exception is the retained raw Keycloak finding with
this exact tuple:
...
`release-candidate` tags are not RC77 references and do not authorize current
...
changed. It is not permitted for RC77, whose PostgreSQL policy is zero findings
...
production OIDC/MFA/logout canary. The current RC77 path is defined only by the
exact tuple and refreshed base digest in the bounded RC77 section at the top of
this document.
