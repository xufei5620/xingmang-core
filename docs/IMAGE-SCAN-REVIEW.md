...
> They are not renamed or rewritten as RC75 evidence.  The current RC75 section
...
## Current RC75 remediation and review status (source/static only)

RC75 is an urgent remediation slice, not an approved release.  The RC48
...
amended, or used as RC75 evidence. RC75 requires a new exact tag, nine newly
...
as read-only and never resume, reuse, amend, rename, or represent it as RC75
evidence.
...
resume, amend, rename, reuse, or represent it as RC75 evidence.
...
### Actual RC75 source changes awaiting evidence
...
- Keycloak refreshes its base to
  ...
  from the existing hardening; it is required evidence, not an RC75 source
  remediation.

The intended RC75 inventory is nine manifest-bound images under one exact,
...
must fail closed.  This inventory is a source contract until the RC75 release
gate produces and independently verifies the manifest and its IDs.

### RC75 exception policy

PostgreSQL has **no RC75 exception**.  Its policy is zero findings: every old
...
The sole permitted RC75 exception is the retained raw Keycloak finding with
this exact tuple:
...
`release-candidate` tags are not RC75 references and do not authorize current
...
changed. It is not permitted for RC75, whose PostgreSQL policy is zero findings
...
production OIDC/MFA/logout canary. The current RC75 path is defined only by the
exact tuple and refreshed base digest in the bounded RC75 section at the top of
this document.
