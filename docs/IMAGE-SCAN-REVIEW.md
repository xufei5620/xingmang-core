...
> They are not renamed or rewritten as RC90 evidence.  The current RC90 section
...
## Current RC90 remediation and review status (source/static only)

RC90 is an urgent remediation slice, not an approved release.  The RC48
...
amended, or used as RC90 evidence. RC90 requires a new exact tag, nine newly
...
as read-only and never resume, reuse, amend, rename, or represent it as RC90
evidence.
...
resume, amend, rename, reuse, or represent it as RC90 evidence.
...
### Actual RC90 source changes awaiting evidence
...
- Keycloak refreshes its base to
  ...
  from the existing hardening; it is required evidence, not an RC90 source
  remediation.

The intended RC90 inventory is nine manifest-bound images under one exact,
...
must fail closed.  This inventory is a source contract until the RC90 release
gate produces and independently verifies the manifest and its IDs.

### RC90 exception policy

PostgreSQL has **no RC90 exception**.  Its policy is zero findings: every old
...
The sole permitted RC90 exception is the retained raw Keycloak finding with
this exact tuple:
...
`release-candidate` tags are not RC90 references and do not authorize current
...
changed. It is not permitted for RC90, whose PostgreSQL policy is zero findings
...
production OIDC/MFA/logout canary. The current RC90 path is defined only by the
exact tuple and refreshed base digest in the bounded RC90 section at the top of
this document.
