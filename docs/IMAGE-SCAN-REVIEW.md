...
> They are not renamed or rewritten as RC68 evidence.  The current RC68 section
...
## Current RC68 remediation and review status (source/static only)

RC68 is an urgent remediation slice, not an approved release.  The RC48
...
amended, or used as RC68 evidence. RC68 requires a new exact tag, nine newly
...
as read-only and never resume, reuse, amend, rename, or represent it as RC68
evidence.
...
resume, amend, rename, reuse, or represent it as RC68 evidence.
...
### Actual RC68 source changes awaiting evidence
...
- Keycloak refreshes its base to
  ...
  from the existing hardening; it is required evidence, not an RC68 source
  remediation.

The intended RC68 inventory is nine manifest-bound images under one exact,
...
must fail closed.  This inventory is a source contract until the RC68 release
gate produces and independently verifies the manifest and its IDs.

### RC68 exception policy

PostgreSQL has **no RC68 exception**.  Its policy is zero findings: every old
...
The sole permitted RC68 exception is the retained raw Keycloak finding with
this exact tuple:
...
`release-candidate` tags are not RC68 references and do not authorize current
...
changed. It is not permitted for RC68, whose PostgreSQL policy is zero findings
...
production OIDC/MFA/logout canary. The current RC68 path is defined only by the
exact tuple and refreshed base digest in the bounded RC68 section at the top of
this document.
