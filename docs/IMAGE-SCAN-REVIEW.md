...
> They are not renamed or rewritten as RC58 evidence.  The current RC58 section
...
## Current RC58 remediation and review status (source/static only)

RC58 is an urgent remediation slice, not an approved release.  The RC48
...
amended, or used as RC58 evidence. RC58 requires a new exact tag, nine newly
...
as read-only and never resume, reuse, amend, rename, or represent it as RC58
evidence.
...
resume, amend, rename, reuse, or represent it as RC58 evidence.
...
### Actual RC58 source changes awaiting evidence
...
- Keycloak refreshes its base to
  ...
  from the existing hardening; it is required evidence, not an RC58 source
  remediation.

The intended RC58 inventory is nine manifest-bound images under one exact,
...
must fail closed.  This inventory is a source contract until the RC58 release
gate produces and independently verifies the manifest and its IDs.

### RC58 exception policy

PostgreSQL has **no RC58 exception**.  Its policy is zero findings: every old
...
The sole permitted RC58 exception is the retained raw Keycloak finding with
this exact tuple:
...
`release-candidate` tags are not RC58 references and do not authorize current
...
changed. It is not permitted for RC58, whose PostgreSQL policy is zero findings
...
production OIDC/MFA/logout canary. The current RC58 path is defined only by the
exact tuple and refreshed base digest in the bounded RC58 section at the top of
this document.
