...
> They are not renamed or rewritten as RC94 evidence.  The current RC94 section
...
## Current RC94 remediation and review status (source/static only)

RC94 is an urgent remediation slice, not an approved release.  The RC48
...
amended, or used as RC94 evidence. RC94 requires a new exact tag, nine newly
...
as read-only and never resume, reuse, amend, rename, or represent it as RC94
evidence.
...
resume, amend, rename, reuse, or represent it as RC94 evidence.
...
### Actual RC94 source changes awaiting evidence
...
- Keycloak refreshes its base to
  ...
  from the existing hardening; it is required evidence, not an RC94 source
  remediation.

The intended RC94 inventory is nine manifest-bound images under one exact,
...
must fail closed.  This inventory is a source contract until the RC94 release
gate produces and independently verifies the manifest and its IDs.

### RC94 exception policy

PostgreSQL has **no RC94 exception**.  Its policy is zero findings: every old
...
The sole permitted RC94 exception is the retained raw Keycloak finding with
this exact tuple:
...
`release-candidate` tags are not RC94 references and do not authorize current
...
changed. It is not permitted for RC94, whose PostgreSQL policy is zero findings
...
production OIDC/MFA/logout canary. The current RC94 path is defined only by the
exact tuple and refreshed base digest in the bounded RC94 section at the top of
this document.
