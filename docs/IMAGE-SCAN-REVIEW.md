...
> They are not renamed or rewritten as RC53 evidence.  The current RC53 section
...
## Current RC53 remediation and review status (source/static only)

RC53 is an urgent remediation slice, not an approved release.  The RC48
...
amended, or used as RC53 evidence. RC53 requires a new exact tag, nine newly
...
as read-only and never resume, reuse, amend, rename, or represent it as RC53
evidence.
...
resume, amend, rename, reuse, or represent it as RC53 evidence.
...
### Actual RC53 source changes awaiting evidence
...
- Keycloak refreshes its base to
  ...
  from the existing hardening; it is required evidence, not an RC53 source
  remediation.

The intended RC53 inventory is nine manifest-bound images under one exact,
...
must fail closed.  This inventory is a source contract until the RC53 release
gate produces and independently verifies the manifest and its IDs.

### RC53 exception policy

PostgreSQL has **no RC53 exception**.  Its policy is zero findings: every old
...
The sole permitted RC53 exception is the retained raw Keycloak finding with
this exact tuple:
...
`release-candidate` tags are not RC53 references and do not authorize current
...
changed. It is not permitted for RC53, whose PostgreSQL policy is zero findings
...
production OIDC/MFA/logout canary. The current RC53 path is defined only by the
exact tuple and refreshed base digest in the bounded RC53 section at the top of
this document.
