...
> They are not renamed or rewritten as RC60 evidence.  The current RC60 section
...
## Current RC60 remediation and review status (source/static only)

RC60 is an urgent remediation slice, not an approved release.  The RC48
...
amended, or used as RC60 evidence. RC60 requires a new exact tag, nine newly
...
as read-only and never resume, reuse, amend, rename, or represent it as RC60
evidence.
...
resume, amend, rename, reuse, or represent it as RC60 evidence.
...
### Actual RC60 source changes awaiting evidence
...
- Keycloak refreshes its base to
  ...
  from the existing hardening; it is required evidence, not an RC60 source
  remediation.

The intended RC60 inventory is nine manifest-bound images under one exact,
...
must fail closed.  This inventory is a source contract until the RC60 release
gate produces and independently verifies the manifest and its IDs.

### RC60 exception policy

PostgreSQL has **no RC60 exception**.  Its policy is zero findings: every old
...
The sole permitted RC60 exception is the retained raw Keycloak finding with
this exact tuple:
...
`release-candidate` tags are not RC60 references and do not authorize current
...
changed. It is not permitted for RC60, whose PostgreSQL policy is zero findings
...
production OIDC/MFA/logout canary. The current RC60 path is defined only by the
exact tuple and refreshed base digest in the bounded RC60 section at the top of
this document.
