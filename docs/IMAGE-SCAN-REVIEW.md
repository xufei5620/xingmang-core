...
> They are not renamed or rewritten as RC64 evidence.  The current RC64 section
...
## Current RC64 remediation and review status (source/static only)

RC64 is an urgent remediation slice, not an approved release.  The RC48
...
amended, or used as RC64 evidence. RC64 requires a new exact tag, nine newly
...
as read-only and never resume, reuse, amend, rename, or represent it as RC64
evidence.
...
resume, amend, rename, reuse, or represent it as RC64 evidence.
...
### Actual RC64 source changes awaiting evidence
...
- Keycloak refreshes its base to
  ...
  from the existing hardening; it is required evidence, not an RC64 source
  remediation.

The intended RC64 inventory is nine manifest-bound images under one exact,
...
must fail closed.  This inventory is a source contract until the RC64 release
gate produces and independently verifies the manifest and its IDs.

### RC64 exception policy

PostgreSQL has **no RC64 exception**.  Its policy is zero findings: every old
...
The sole permitted RC64 exception is the retained raw Keycloak finding with
this exact tuple:
...
`release-candidate` tags are not RC64 references and do not authorize current
...
changed. It is not permitted for RC64, whose PostgreSQL policy is zero findings
...
production OIDC/MFA/logout canary. The current RC64 path is defined only by the
exact tuple and refreshed base digest in the bounded RC64 section at the top of
this document.
