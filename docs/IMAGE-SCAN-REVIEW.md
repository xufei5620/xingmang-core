...
> They are not renamed or rewritten as RC61 evidence.  The current RC61 section
...
## Current RC61 remediation and review status (source/static only)

RC61 is an urgent remediation slice, not an approved release.  The RC48
...
amended, or used as RC61 evidence. RC61 requires a new exact tag, nine newly
...
as read-only and never resume, reuse, amend, rename, or represent it as RC61
evidence.
...
resume, amend, rename, reuse, or represent it as RC61 evidence.
...
### Actual RC61 source changes awaiting evidence
...
- Keycloak refreshes its base to
  ...
  from the existing hardening; it is required evidence, not an RC61 source
  remediation.

The intended RC61 inventory is nine manifest-bound images under one exact,
...
must fail closed.  This inventory is a source contract until the RC61 release
gate produces and independently verifies the manifest and its IDs.

### RC61 exception policy

PostgreSQL has **no RC61 exception**.  Its policy is zero findings: every old
...
The sole permitted RC61 exception is the retained raw Keycloak finding with
this exact tuple:
...
`release-candidate` tags are not RC61 references and do not authorize current
...
changed. It is not permitted for RC61, whose PostgreSQL policy is zero findings
...
production OIDC/MFA/logout canary. The current RC61 path is defined only by the
exact tuple and refreshed base digest in the bounded RC61 section at the top of
this document.
