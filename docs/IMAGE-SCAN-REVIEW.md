...
> They are not renamed or rewritten as RC98 evidence.  The current RC98 section
...
## Current RC98 remediation and review status (source/static only)

RC98 is an urgent remediation slice, not an approved release.  The RC48
...
amended, or used as RC98 evidence. RC98 requires a new exact tag, nine newly
...
as read-only and never resume, reuse, amend, rename, or represent it as RC98
evidence.
...
resume, amend, rename, reuse, or represent it as RC98 evidence.
...
### Actual RC98 source changes awaiting evidence
...
- Keycloak refreshes its base to
  ...
  from the existing hardening; it is required evidence, not an RC98 source
  remediation.

The intended RC98 inventory is nine manifest-bound images under one exact,
...
must fail closed.  This inventory is a source contract until the RC98 release
gate produces and independently verifies the manifest and its IDs.

### RC98 exception policy

PostgreSQL has **no RC98 exception**.  Its policy is zero findings: every old
...
The sole permitted RC98 exception is the retained raw Keycloak finding with
this exact tuple:
...
`release-candidate` tags are not RC98 references and do not authorize current
...
changed. It is not permitted for RC98, whose PostgreSQL policy is zero findings
...
production OIDC/MFA/logout canary. The current RC98 path is defined only by the
exact tuple and refreshed base digest in the bounded RC98 section at the top of
this document.
