...
> They are not renamed or rewritten as RC72 evidence.  The current RC72 section
...
## Current RC72 remediation and review status (source/static only)

RC72 is an urgent remediation slice, not an approved release.  The RC48
...
amended, or used as RC72 evidence. RC72 requires a new exact tag, nine newly
...
as read-only and never resume, reuse, amend, rename, or represent it as RC72
evidence.
...
resume, amend, rename, reuse, or represent it as RC72 evidence.
...
### Actual RC72 source changes awaiting evidence
...
- Keycloak refreshes its base to
  ...
  from the existing hardening; it is required evidence, not an RC72 source
  remediation.

The intended RC72 inventory is nine manifest-bound images under one exact,
...
must fail closed.  This inventory is a source contract until the RC72 release
gate produces and independently verifies the manifest and its IDs.

### RC72 exception policy

PostgreSQL has **no RC72 exception**.  Its policy is zero findings: every old
...
The sole permitted RC72 exception is the retained raw Keycloak finding with
this exact tuple:
...
`release-candidate` tags are not RC72 references and do not authorize current
...
changed. It is not permitted for RC72, whose PostgreSQL policy is zero findings
...
production OIDC/MFA/logout canary. The current RC72 path is defined only by the
exact tuple and refreshed base digest in the bounded RC72 section at the top of
this document.
