...
> They are not renamed or rewritten as RC85 evidence.  The current RC85 section
...
## Current RC85 remediation and review status (source/static only)

RC85 is an urgent remediation slice, not an approved release.  The RC48
...
amended, or used as RC85 evidence. RC85 requires a new exact tag, nine newly
...
as read-only and never resume, reuse, amend, rename, or represent it as RC85
evidence.
...
resume, amend, rename, reuse, or represent it as RC85 evidence.
...
### Actual RC85 source changes awaiting evidence
...
- Keycloak refreshes its base to
  ...
  from the existing hardening; it is required evidence, not an RC85 source
  remediation.

The intended RC85 inventory is nine manifest-bound images under one exact,
...
must fail closed.  This inventory is a source contract until the RC85 release
gate produces and independently verifies the manifest and its IDs.

### RC85 exception policy

PostgreSQL has **no RC85 exception**.  Its policy is zero findings: every old
...
The sole permitted RC85 exception is the retained raw Keycloak finding with
this exact tuple:
...
`release-candidate` tags are not RC85 references and do not authorize current
...
changed. It is not permitted for RC85, whose PostgreSQL policy is zero findings
...
production OIDC/MFA/logout canary. The current RC85 path is defined only by the
exact tuple and refreshed base digest in the bounded RC85 section at the top of
this document.
