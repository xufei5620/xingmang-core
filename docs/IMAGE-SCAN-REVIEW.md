...
> They are not renamed or rewritten as RC74 evidence.  The current RC74 section
...
## Current RC74 remediation and review status (source/static only)

RC74 is an urgent remediation slice, not an approved release.  The RC48
...
amended, or used as RC74 evidence. RC74 requires a new exact tag, nine newly
...
as read-only and never resume, reuse, amend, rename, or represent it as RC74
evidence.
...
resume, amend, rename, reuse, or represent it as RC74 evidence.
...
### Actual RC74 source changes awaiting evidence
...
- Keycloak refreshes its base to
  ...
  from the existing hardening; it is required evidence, not an RC74 source
  remediation.

The intended RC74 inventory is nine manifest-bound images under one exact,
...
must fail closed.  This inventory is a source contract until the RC74 release
gate produces and independently verifies the manifest and its IDs.

### RC74 exception policy

PostgreSQL has **no RC74 exception**.  Its policy is zero findings: every old
...
The sole permitted RC74 exception is the retained raw Keycloak finding with
this exact tuple:
...
`release-candidate` tags are not RC74 references and do not authorize current
...
changed. It is not permitted for RC74, whose PostgreSQL policy is zero findings
...
production OIDC/MFA/logout canary. The current RC74 path is defined only by the
exact tuple and refreshed base digest in the bounded RC74 section at the top of
this document.
