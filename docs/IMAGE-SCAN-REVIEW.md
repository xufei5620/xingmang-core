...
> They are not renamed or rewritten as RC54 evidence.  The current RC54 section
...
## Current RC54 remediation and review status (source/static only)

RC54 is an urgent remediation slice, not an approved release.  The RC48
...
amended, or used as RC54 evidence. RC54 requires a new exact tag, nine newly
...
as read-only and never resume, reuse, amend, rename, or represent it as RC54
evidence.
...
resume, amend, rename, reuse, or represent it as RC54 evidence.
...
### Actual RC54 source changes awaiting evidence
...
- Keycloak refreshes its base to
  ...
  from the existing hardening; it is required evidence, not an RC54 source
  remediation.

The intended RC54 inventory is nine manifest-bound images under one exact,
...
must fail closed.  This inventory is a source contract until the RC54 release
gate produces and independently verifies the manifest and its IDs.

### RC54 exception policy

PostgreSQL has **no RC54 exception**.  Its policy is zero findings: every old
...
The sole permitted RC54 exception is the retained raw Keycloak finding with
this exact tuple:
...
`release-candidate` tags are not RC54 references and do not authorize current
...
changed. It is not permitted for RC54, whose PostgreSQL policy is zero findings
...
production OIDC/MFA/logout canary. The current RC54 path is defined only by the
exact tuple and refreshed base digest in the bounded RC54 section at the top of
this document.
