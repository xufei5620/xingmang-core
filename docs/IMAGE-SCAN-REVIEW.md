...
> They are not renamed or rewritten as RC84 evidence.  The current RC84 section
...
## Current RC84 remediation and review status (source/static only)

RC84 is an urgent remediation slice, not an approved release.  The RC48
...
amended, or used as RC84 evidence. RC84 requires a new exact tag, nine newly
...
as read-only and never resume, reuse, amend, rename, or represent it as RC84
evidence.
...
resume, amend, rename, reuse, or represent it as RC84 evidence.
...
### Actual RC84 source changes awaiting evidence
...
- Keycloak refreshes its base to
  ...
  from the existing hardening; it is required evidence, not an RC84 source
  remediation.

The intended RC84 inventory is nine manifest-bound images under one exact,
...
must fail closed.  This inventory is a source contract until the RC84 release
gate produces and independently verifies the manifest and its IDs.

### RC84 exception policy

PostgreSQL has **no RC84 exception**.  Its policy is zero findings: every old
...
The sole permitted RC84 exception is the retained raw Keycloak finding with
this exact tuple:
...
`release-candidate` tags are not RC84 references and do not authorize current
...
changed. It is not permitted for RC84, whose PostgreSQL policy is zero findings
...
production OIDC/MFA/logout canary. The current RC84 path is defined only by the
exact tuple and refreshed base digest in the bounded RC84 section at the top of
this document.
