...
> They are not renamed or rewritten as RC76 evidence.  The current RC76 section
...
## Current RC76 remediation and review status (source/static only)

RC76 is an urgent remediation slice, not an approved release.  The RC48
...
amended, or used as RC76 evidence. RC76 requires a new exact tag, nine newly
...
as read-only and never resume, reuse, amend, rename, or represent it as RC76
evidence.
...
resume, amend, rename, reuse, or represent it as RC76 evidence.
...
### Actual RC76 source changes awaiting evidence
...
- Keycloak refreshes its base to
  ...
  from the existing hardening; it is required evidence, not an RC76 source
  remediation.

The intended RC76 inventory is nine manifest-bound images under one exact,
...
must fail closed.  This inventory is a source contract until the RC76 release
gate produces and independently verifies the manifest and its IDs.

### RC76 exception policy

PostgreSQL has **no RC76 exception**.  Its policy is zero findings: every old
...
The sole permitted RC76 exception is the retained raw Keycloak finding with
this exact tuple:
...
`release-candidate` tags are not RC76 references and do not authorize current
...
changed. It is not permitted for RC76, whose PostgreSQL policy is zero findings
...
production OIDC/MFA/logout canary. The current RC76 path is defined only by the
exact tuple and refreshed base digest in the bounded RC76 section at the top of
this document.
