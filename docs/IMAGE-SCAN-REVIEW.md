...
> They are not renamed or rewritten as RC62 evidence.  The current RC62 section
...
## Current RC62 remediation and review status (source/static only)

RC62 is an urgent remediation slice, not an approved release.  The RC48
...
amended, or used as RC62 evidence. RC62 requires a new exact tag, nine newly
...
as read-only and never resume, reuse, amend, rename, or represent it as RC62
evidence.
...
resume, amend, rename, reuse, or represent it as RC62 evidence.
...
### Actual RC62 source changes awaiting evidence
...
- Keycloak refreshes its base to
  ...
  from the existing hardening; it is required evidence, not an RC62 source
  remediation.

The intended RC62 inventory is nine manifest-bound images under one exact,
...
must fail closed.  This inventory is a source contract until the RC62 release
gate produces and independently verifies the manifest and its IDs.

### RC62 exception policy

PostgreSQL has **no RC62 exception**.  Its policy is zero findings: every old
...
The sole permitted RC62 exception is the retained raw Keycloak finding with
this exact tuple:
...
`release-candidate` tags are not RC62 references and do not authorize current
...
changed. It is not permitted for RC62, whose PostgreSQL policy is zero findings
...
production OIDC/MFA/logout canary. The current RC62 path is defined only by the
exact tuple and refreshed base digest in the bounded RC62 section at the top of
this document.
