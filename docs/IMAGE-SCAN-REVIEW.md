...
> They are not renamed or rewritten as RC56 evidence.  The current RC56 section
...
## Current RC56 remediation and review status (source/static only)

RC56 is an urgent remediation slice, not an approved release.  The RC48
...
amended, or used as RC56 evidence. RC56 requires a new exact tag, nine newly
...
as read-only and never resume, reuse, amend, rename, or represent it as RC56
evidence.
...
resume, amend, rename, reuse, or represent it as RC56 evidence.
...
### Actual RC56 source changes awaiting evidence
...
- Keycloak refreshes its base to
  ...
  from the existing hardening; it is required evidence, not an RC56 source
  remediation.

The intended RC56 inventory is nine manifest-bound images under one exact,
...
must fail closed.  This inventory is a source contract until the RC56 release
gate produces and independently verifies the manifest and its IDs.

### RC56 exception policy

PostgreSQL has **no RC56 exception**.  Its policy is zero findings: every old
...
The sole permitted RC56 exception is the retained raw Keycloak finding with
this exact tuple:
...
`release-candidate` tags are not RC56 references and do not authorize current
...
changed. It is not permitted for RC56, whose PostgreSQL policy is zero findings
...
production OIDC/MFA/logout canary. The current RC56 path is defined only by the
exact tuple and refreshed base digest in the bounded RC56 section at the top of
this document.
