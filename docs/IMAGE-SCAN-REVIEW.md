...
> They are not renamed or rewritten as RC69 evidence.  The current RC69 section
...
## Current RC69 remediation and review status (source/static only)

RC69 is an urgent remediation slice, not an approved release.  The RC48
...
amended, or used as RC69 evidence. RC69 requires a new exact tag, nine newly
...
as read-only and never resume, reuse, amend, rename, or represent it as RC69
evidence.
...
resume, amend, rename, reuse, or represent it as RC69 evidence.
...
### Actual RC69 source changes awaiting evidence
...
- Keycloak refreshes its base to
  ...
  from the existing hardening; it is required evidence, not an RC69 source
  remediation.

The intended RC69 inventory is nine manifest-bound images under one exact,
...
must fail closed.  This inventory is a source contract until the RC69 release
gate produces and independently verifies the manifest and its IDs.

### RC69 exception policy

PostgreSQL has **no RC69 exception**.  Its policy is zero findings: every old
...
The sole permitted RC69 exception is the retained raw Keycloak finding with
this exact tuple:
...
`release-candidate` tags are not RC69 references and do not authorize current
...
changed. It is not permitted for RC69, whose PostgreSQL policy is zero findings
...
production OIDC/MFA/logout canary. The current RC69 path is defined only by the
exact tuple and refreshed base digest in the bounded RC69 section at the top of
this document.
