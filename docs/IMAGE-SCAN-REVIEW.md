...
> They are not renamed or rewritten as RC55 evidence.  The current RC55 section
...
## Current RC55 remediation and review status (source/static only)

RC55 is an urgent remediation slice, not an approved release.  The RC48
...
amended, or used as RC55 evidence. RC55 requires a new exact tag, nine newly
...
as read-only and never resume, reuse, amend, rename, or represent it as RC55
evidence.
...
resume, amend, rename, reuse, or represent it as RC55 evidence.
...
### Actual RC55 source changes awaiting evidence
...
- Keycloak refreshes its base to
  ...
  from the existing hardening; it is required evidence, not an RC55 source
  remediation.

The intended RC55 inventory is nine manifest-bound images under one exact,
...
must fail closed.  This inventory is a source contract until the RC55 release
gate produces and independently verifies the manifest and its IDs.

### RC55 exception policy

PostgreSQL has **no RC55 exception**.  Its policy is zero findings: every old
...
The sole permitted RC55 exception is the retained raw Keycloak finding with
this exact tuple:
...
`release-candidate` tags are not RC55 references and do not authorize current
...
changed. It is not permitted for RC55, whose PostgreSQL policy is zero findings
...
production OIDC/MFA/logout canary. The current RC55 path is defined only by the
exact tuple and refreshed base digest in the bounded RC55 section at the top of
this document.
