...
> They are not renamed or rewritten as RC63 evidence.  The current RC63 section
...
## Current RC63 remediation and review status (source/static only)

RC63 is an urgent remediation slice, not an approved release.  The RC48
...
amended, or used as RC63 evidence. RC63 requires a new exact tag, nine newly
...
as read-only and never resume, reuse, amend, rename, or represent it as RC63
evidence.
...
resume, amend, rename, reuse, or represent it as RC63 evidence.
...
### Actual RC63 source changes awaiting evidence
...
- Keycloak refreshes its base to
  ...
  from the existing hardening; it is required evidence, not an RC63 source
  remediation.

The intended RC63 inventory is nine manifest-bound images under one exact,
...
must fail closed.  This inventory is a source contract until the RC63 release
gate produces and independently verifies the manifest and its IDs.

### RC63 exception policy

PostgreSQL has **no RC63 exception**.  Its policy is zero findings: every old
...
The sole permitted RC63 exception is the retained raw Keycloak finding with
this exact tuple:
...
`release-candidate` tags are not RC63 references and do not authorize current
...
changed. It is not permitted for RC63, whose PostgreSQL policy is zero findings
...
production OIDC/MFA/logout canary. The current RC63 path is defined only by the
exact tuple and refreshed base digest in the bounded RC63 section at the top of
this document.
