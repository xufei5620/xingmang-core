...
> They are not renamed or rewritten as RC78 evidence.  The current RC78 section
...
## Current RC78 remediation and review status (source/static only)

RC78 is an urgent remediation slice, not an approved release.  The RC48
...
amended, or used as RC78 evidence. RC78 requires a new exact tag, nine newly
...
as read-only and never resume, reuse, amend, rename, or represent it as RC78
evidence.
...
resume, amend, rename, reuse, or represent it as RC78 evidence.
...
### Actual RC78 source changes awaiting evidence
...
- Keycloak refreshes its base to
  ...
  from the existing hardening; it is required evidence, not an RC78 source
  remediation.

The intended RC78 inventory is nine manifest-bound images under one exact,
...
must fail closed.  This inventory is a source contract until the RC78 release
gate produces and independently verifies the manifest and its IDs.

### RC78 exception policy

PostgreSQL has **no RC78 exception**.  Its policy is zero findings: every old
...
The sole permitted RC78 exception is the retained raw Keycloak finding with
this exact tuple:
...
`release-candidate` tags are not RC78 references and do not authorize current
...
changed. It is not permitted for RC78, whose PostgreSQL policy is zero findings
...
production OIDC/MFA/logout canary. The current RC78 path is defined only by the
exact tuple and refreshed base digest in the bounded RC78 section at the top of
this document.
