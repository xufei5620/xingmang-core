...
> They are not renamed or rewritten as RC93 evidence.  The current RC93 section
...
## Current RC93 remediation and review status (source/static only)

RC93 is an urgent remediation slice, not an approved release.  The RC48
...
amended, or used as RC93 evidence. RC93 requires a new exact tag, nine newly
...
as read-only and never resume, reuse, amend, rename, or represent it as RC93
evidence.
...
resume, amend, rename, reuse, or represent it as RC93 evidence.
...
### Actual RC93 source changes awaiting evidence
...
- Keycloak refreshes its base to
  ...
  from the existing hardening; it is required evidence, not an RC93 source
  remediation.

The intended RC93 inventory is nine manifest-bound images under one exact,
...
must fail closed.  This inventory is a source contract until the RC93 release
gate produces and independently verifies the manifest and its IDs.

### RC93 exception policy

PostgreSQL has **no RC93 exception**.  Its policy is zero findings: every old
...
The sole permitted RC93 exception is the retained raw Keycloak finding with
this exact tuple:
...
`release-candidate` tags are not RC93 references and do not authorize current
...
changed. It is not permitted for RC93, whose PostgreSQL policy is zero findings
...
production OIDC/MFA/logout canary. The current RC93 path is defined only by the
exact tuple and refreshed base digest in the bounded RC93 section at the top of
this document.
