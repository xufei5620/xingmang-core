...
> They are not renamed or rewritten as RC91 evidence.  The current RC91 section
...
## Current RC91 remediation and review status (source/static only)

RC91 is an urgent remediation slice, not an approved release.  The RC48
...
amended, or used as RC91 evidence. RC91 requires a new exact tag, nine newly
...
as read-only and never resume, reuse, amend, rename, or represent it as RC91
evidence.
...
resume, amend, rename, reuse, or represent it as RC91 evidence.
...
### Actual RC91 source changes awaiting evidence
...
- Keycloak refreshes its base to
  ...
  from the existing hardening; it is required evidence, not an RC91 source
  remediation.

The intended RC91 inventory is nine manifest-bound images under one exact,
...
must fail closed.  This inventory is a source contract until the RC91 release
gate produces and independently verifies the manifest and its IDs.

### RC91 exception policy

PostgreSQL has **no RC91 exception**.  Its policy is zero findings: every old
...
The sole permitted RC91 exception is the retained raw Keycloak finding with
this exact tuple:
...
`release-candidate` tags are not RC91 references and do not authorize current
...
changed. It is not permitted for RC91, whose PostgreSQL policy is zero findings
...
production OIDC/MFA/logout canary. The current RC91 path is defined only by the
exact tuple and refreshed base digest in the bounded RC91 section at the top of
this document.
