...
> They are not renamed or rewritten as RC80 evidence.  The current RC80 section
...
## Current RC80 remediation and review status (source/static only)

RC80 is an urgent remediation slice, not an approved release.  The RC48
...
amended, or used as RC80 evidence. RC80 requires a new exact tag, nine newly
...
as read-only and never resume, reuse, amend, rename, or represent it as RC80
evidence.
...
resume, amend, rename, reuse, or represent it as RC80 evidence.
...
### Actual RC80 source changes awaiting evidence
...
- Keycloak refreshes its base to
  ...
  from the existing hardening; it is required evidence, not an RC80 source
  remediation.

The intended RC80 inventory is nine manifest-bound images under one exact,
...
must fail closed.  This inventory is a source contract until the RC80 release
gate produces and independently verifies the manifest and its IDs.

### RC80 exception policy

PostgreSQL has **no RC80 exception**.  Its policy is zero findings: every old
...
The sole permitted RC80 exception is the retained raw Keycloak finding with
this exact tuple:
...
`release-candidate` tags are not RC80 references and do not authorize current
...
changed. It is not permitted for RC80, whose PostgreSQL policy is zero findings
...
production OIDC/MFA/logout canary. The current RC80 path is defined only by the
exact tuple and refreshed base digest in the bounded RC80 section at the top of
this document.
