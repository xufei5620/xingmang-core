...
1. verify signed tag `v0.1.0-rc17-signed` peels to commit
   ...
   manifest's exact rollback image IDs; verify the exact RC22 candidate images,
...
- Existing pair: do not recapture it. With the exact RC22 source-agent image,
...
- First installation with no pair: before applying 0011, stop one upstream
  ...
  RC22 source-agent image to capture that source once and immediately run
  ...
  and restore-test the full backup in explicit RC22 pre-0011 mode while the
...
INVOICE_TOOLS_IMAGE='<exact RC22 tools image>' \
SOURCE_AGENT_IMAGE='<exact RC22 source-agent image>' \
