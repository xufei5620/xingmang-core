...
Run locally from the exact RC68 candidate worktree
`K:\发票\wt-XM-INV-AUTOLOGIN`:

```powershell
pwsh -NoProfile -File .\scripts\verify.ps1
$sourceGateExit = $LASTEXITCODE
if ($sourceGateExit -ne 0) { throw "RC68 full source gate failed with exit $sourceGateExit" }
```
...
transfer them as RC68 evidence.
...
Before copying images to the server, begin from and verify the clean signed RC68
...
if ($worktreeExit -ne 0 -or -not [string]::Equals(($worktreeLines -join '').Trim(), $expectedWorktree, [StringComparison]::OrdinalIgnoreCase)) { throw 'wrong RC68 candidate worktree' }
...
if ($gitleaksExit -ne 0) { throw "RC68 release-range gitleaks failed with exit $gitleaksExit" }
git diff --check
$diffExit = $LASTEXITCODE
if ($diffExit -ne 0) { throw "RC68 source diff check failed with exit $diffExit" }
$statusLines = @(git status --porcelain=v1)
$statusExit = $LASTEXITCODE
if ($statusExit -ne 0) { throw "RC68 git status failed with exit $statusExit" }
if ($statusLines.Count -ne 0) { throw 'RC68 source worktree is dirty' }
git verify-commit HEAD
$commitVerifyExit = $LASTEXITCODE
if ($commitVerifyExit -ne 0) { throw "RC68 source commit signature verification failed with exit $commitVerifyExit" }
git tag -s -a v0.1.0-rc68-signed -m 'RC68 image-security release candidate'
$tagCreateExit = $LASTEXITCODE
if ($tagCreateExit -ne 0) { throw "RC68 signed tag creation failed with exit $tagCreateExit" }
git verify-tag refs/tags/v0.1.0-rc68-signed
$tagVerifyExit = $LASTEXITCODE
if ($tagVerifyExit -ne 0) { throw "RC68 tag signature verification failed with exit $tagVerifyExit" }
$tagHeadLines = @(git rev-parse --verify 'refs/tags/v0.1.0-rc68-signed^{}')
$tagHeadExit = $LASTEXITCODE
$headLines = @(git rev-parse --verify HEAD)
$headExit = $LASTEXITCODE
if ($tagHeadExit -ne 0 -or $headExit -ne 0 -or ($tagHeadLines -join '').Trim() -cne ($headLines -join '').Trim()) { throw 'RC68 signed tag does not peel to candidate HEAD' }
```

Then complete the RC68 image gate from that exact signed source.  It builds all
...
```powershell
# Creates new RC68 evidence; do not reuse or overwrite RC48 through RC52 evidence.
$worktreeLines = @(git rev-parse --show-toplevel)
$worktreeExit = $LASTEXITCODE
$headLines = @(git rev-parse --verify HEAD)
$headExit = $LASTEXITCODE
...
if ($worktreeExit -ne 0 -or $headExit -ne 0 -or $tagHeadExit -ne 0 -or
    ...
    ($headLines -join '').Trim() -cne ($tagHeadLines -join '').Trim()) { throw 'RC68 worktree/tag/HEAD binding failed' }
$rc68ReleaseDirectory = 1..99 |
  ForEach-Object { "release\0.1.0-rc68-exact$_" } |
  Where-Object { -not (Test-Path -LiteralPath $_) } |
  Select-Object -First 1
if ([string]::IsNullOrWhiteSpace($rc68ReleaseDirectory)) { throw 'no unused RC68 exact directory remains' }
pwsh -NoProfile -File .\scripts\release-image-gate.ps1 `
  -ReleaseName 0.1.0-rc68 `
  -ImageTag 0.1.0-rc68 `
  -SourceAgentVersion 0.3.0 `
  -ReleaseDirectory $rc68ReleaseDirectory `
  -IdPMode keycloak
$imageGateExit = $LASTEXITCODE
if ($imageGateExit -ne 42) { throw "RC68 image gate expected exit 42, got $imageGateExit" }
pwsh -NoProfile -File .\scripts\verify-release-image-artifacts.ps1 -ReleaseDirectory $rc68ReleaseDirectory
$ordinaryVerifyExit = $LASTEXITCODE
if ($ordinaryVerifyExit -ne 0) { throw "ordinary RC68 artifact verification failed with exit $ordinaryVerifyExit" }
pwsh -NoProfile -File .\scripts\verify-release-image-artifacts.ps1 -ReleaseDirectory $rc68ReleaseDirectory -RequireTransferReady -SignedReleaseTag v0.1.0-rc68-signed
...
The one-block RC68 gate above already runs the ordinary verifier immediately
...
lock races. No vulnerability is ignored. RC68 PostgreSQL has no exception and
...
verifier in strict transfer-ready mode against the signed RC68 tag. The
...
commit, `releaseName=0.1.0-rc68`, all nine exact `:0.1.0-rc68` image
...
RC68 permits only the exact Keycloak vendor-rejected tuple documented in
...
foreach ($candidate in Get-ChildItem -LiteralPath release -Directory -Filter '0.1.0-rc68-exact*') {
  pwsh -NoProfile -File .\scripts\verify-release-image-artifacts.ps1 -ReleaseDirectory $candidate.FullName -RequireTransferReady -SignedReleaseTag v0.1.0-rc68-signed
  $candidateVerifyExit = $LASTEXITCODE
  if ($candidateVerifyExit -eq 0) { $strictReadyCandidates.Add($candidate.FullName) }
}
if ($strictReadyCandidates.Count -ne 1) { throw "expected one strict-ready RC68 directory, found $($strictReadyCandidates.Count)" }
...
$releaseSigningKey = '<offline RC68 release Ed25519 private key>'
...
if ($LASTEXITCODE -ne 0 -or -not (Test-Path -LiteralPath $releaseSignature -PathType Leaf)) {
  throw 'RC68 artifact SHA256SUMS signature was not created'
}
...
if ($LASTEXITCODE -ne 0) { throw 'RC68 artifact SHA256SUMS signature verification failed' }
```
...
The RC68 source baselines are Go 1.25.13, pgx 5.9.2, x/text 0.39.0,
...
Set `INVOICE_IMAGE_TAG` in `deploy/.env.production` only after the exact RC68
...
For RC68, continue with sections 4, 5, 6, 7, 8, 9, 10, 11 (excluding 11.1),
12, and 13 in order. Section 3.1 (RC39 one-off), section 3.2 (balance cleanup),
and section 11.1 (quarterly deletion) are outside the RC68 release and require
...
```bash
export INVOICE_IMAGE_TAG='<exact tag from the verified RC68 release manifest>'
: "${RC68_RELEASE_DIRECTORY:?set the exact strict-verified and signed RC68 exactN directory name from the release ticket}"
case "$RC68_RELEASE_DIRECTORY" in 0.1.0-rc68-exact[1-9]|0.1.0-rc68-exact[1-9][0-9]) ;; *) echo 'invalid RC68 release directory' >&2; exit 1 ;; esac
RELEASE_MANIFEST="/root/invoice-system/release/$RC68_RELEASE_DIRECTORY/release-manifest.json"
...
under the backup directory. The RC68 installation must create root-only
...
Before the mail window, rebuild/recreate the exact RC68 Keycloak container and
...
1. verify signed tag `v0.1.0-rc17-signed` peels to commit
   ...
   manifest's exact rollback image IDs; verify the exact RC68 candidate images,
...
3. while they remain stopped, run
   the reviewed RC68 `deploy/backup/backup.sh` with
   `BACKUP_SCHEMA_MODE=pre-0011`; it records initial service state and must not
   start a service that was stopped. Restore it with the RC68 drill and
   ...
   agent; the RC68 V4 agent must not be used to reinterpret it. Do not use the
...
- Existing V3 pair: do not recapture or reinterpret it. With the exact RC24
  ...
  generation only; it is not authorization to start the RC68 receiver.
- First installation with no pair: before applying 0011, stop one upstream
  ...
  RC68 source-agent image to capture that source once and immediately run
  ...
  and restore-test the full backup in explicit RC68 pre-0011 mode while the
...
1. Keep the verified pre-0011 package. Create, sign and restore-test a separate
   ...
   image recorded for that recovery generation, never RC68.
2. Stop all source agents. Install the RC68 v4 semantic-fingerprint functions
...
INVOICE_TOOLS_IMAGE='<exact RC68 tools image>' \
...
to the timestamp of the last payment is an RC68 rollback condition.
