...
1. verify signed tag `v0.1.0-rc17-signed` peels to commit
   ...
   manifest's exact rollback image IDs; verify the exact RC28 candidate images,
...
3. while they remain stopped, run
   the reviewed RC28 `deploy/backup/backup.sh` with
   `BACKUP_SCHEMA_MODE=pre-0011`; it records initial service state and must not
   start a service that was stopped. Restore it with the RC28 drill and
   ...
   agent; the RC28 V4 agent must not be used to reinterpret it. Do not use the
...
- Existing V3 pair: do not recapture or reinterpret it. With the exact RC24
  ...
  generation only; it is not authorization to start the RC28 receiver.
- First installation with no pair: before applying 0011, stop one upstream
  ...
  RC28 source-agent image to capture that source once and immediately run
  ...
  and restore-test the full backup in explicit RC28 pre-0011 mode while the
...
1. Keep the verified pre-0011 package. Create, sign and restore-test a separate
   ...
   image recorded for that recovery generation, never RC28.
2. Stop all source agents. Install the RC28 v4 semantic-fingerprint functions
...
INVOICE_TOOLS_IMAGE='<exact RC28 tools image>' \
