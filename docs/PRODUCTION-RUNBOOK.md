...
Run locally from the exact RC51 candidate worktree
`K:\发票\wt-XM-INV-SEC-RC49`:

```powershell
pwsh -NoProfile -File .\scripts\verify.ps1
$sourceGateExit = $LASTEXITCODE
if ($sourceGateExit -ne 0) { throw "RC51 full source gate failed with exit $sourceGateExit" }
```
...
transfer them as RC51 evidence.
...
Before copying images to the server, begin from and verify the clean signed RC51
...
if ($diffExit -ne 0) { throw "RC51 source diff check failed with exit $diffExit" }
...
if ($commitVerifyExit -ne 0) { throw "RC51 source commit signature verification failed with exit $commitVerifyExit" }
git tag -s -a v0.1.0-rc51-signed -m 'RC51 image-security release candidate'
$tagCreateExit = $LASTEXITCODE
if ($tagCreateExit -ne 0) { throw "RC51 signed tag creation failed with exit $tagCreateExit" }
...
if ($tagHeadExit -ne 0 -or $headExit -ne 0 -or ($tagHeadLines -join '').Trim() -cne ($headLines -join '').Trim()) { throw 'RC51 signed tag does not peel to candidate HEAD' }
```

Then complete the RC51 image gate from that exact signed source.  It builds all
...
```powershell
# Creates new RC51 evidence; do not reuse or overwrite RC48/RC49/RC50 evidence.
$worktreeLines = @(git rev-parse --show-toplevel)
$worktreeExit = $LASTEXITCODE
$headLines = @(git rev-parse --verify HEAD)
$headExit = $LASTEXITCODE
...
$rc51ReleaseDirectory = 1..99 |
  ForEach-Object { "release\0.1.0-rc51-exact$_" } |
...
pwsh -NoProfile -File .\scripts\release-image-gate.ps1 `
  ...
  -ReleaseDirectory $rc51ReleaseDirectory `
  -IdPMode keycloak
...
if ($strictVerifyExit -ne 0) { throw "strict RC51 transfer-ready verification failed with exit $strictVerifyExit" }
```
...
The one-block RC51 gate above already runs the ordinary verifier immediately
...
lock races. No vulnerability is ignored. RC51 PostgreSQL has no exception and
...
verifier in strict transfer-ready mode against the signed RC51 tag. The
...
commit, `releaseName=0.1.0-rc51`, all nine exact `:0.1.0-rc51` image
...
RC51 permits only the exact Keycloak vendor-rejected tuple documented in
...
foreach ($candidate in Get-ChildItem -LiteralPath release -Directory -Filter '0.1.0-rc51-exact*') {
  pwsh -NoProfile -File .\scripts\verify-release-image-artifacts.ps1 -ReleaseDirectory $candidate.FullName -RequireTransferReady -SignedReleaseTag v0.1.0-rc51-signed
  $candidateVerifyExit = $LASTEXITCODE
  if ($candidateVerifyExit -eq 0) { $strictReadyCandidates.Add($candidate.FullName) }
}
if ($strictReadyCandidates.Count -ne 1) { throw "expected one strict-ready RC51 directory, found $($strictReadyCandidates.Count)" }
...
$releaseSigningKey = '<offline RC51 release Ed25519 private key>'
...
if ($LASTEXITCODE -ne 0 -or -not (Test-Path -LiteralPath $releaseSignature -PathType Leaf)) {
  throw 'RC51 artifact SHA256SUMS signature was not created'
}
...
if ($LASTEXITCODE -ne 0) { throw 'RC51 artifact SHA256SUMS signature verification failed' }
```
...
The RC51 source baselines are Go 1.25.13, pgx 5.9.2, x/text 0.39.0,
...
Set `INVOICE_IMAGE_TAG` in `deploy/.env.production` only after the exact RC51
...
For RC51, continue with sections 4, 5, 6, 7, 8, 9, 10, 11 (excluding 11.1),
12, and 13 in order. Section 3.1 (RC39 one-off), section 3.2 (balance cleanup),
and section 11.1 (quarterly deletion) are outside the RC51 release and require
...
```bash
export INVOICE_IMAGE_TAG='<exact tag from the verified RC51 release manifest>'
: "${RC51_RELEASE_DIRECTORY:?set the exact strict-verified and signed RC51 exactN directory name from the release ticket}"
case "$RC51_RELEASE_DIRECTORY" in 0.1.0-rc51-exact[1-9]|0.1.0-rc51-exact[1-9][0-9]) ;; *) echo 'invalid RC51 release directory' >&2; exit 1 ;; esac
RELEASE_MANIFEST="/root/invoice-system/release/$RC51_RELEASE_DIRECTORY/release-manifest.json"
...
under the backup directory. The RC51 installation must create root-only
...
Before the mail window, rebuild/recreate the exact RC51 Keycloak container and
...
1. verify signed tag `v0.1.0-rc17-signed` peels to commit
   ...
   manifest's exact rollback image IDs; verify the exact RC51 candidate images,
...
3. while they remain stopped, run
   the reviewed RC51 `deploy/backup/backup.sh` with
   `BACKUP_SCHEMA_MODE=pre-0011`; it records initial service state and must not
   start a service that was stopped. Restore it with the RC51 drill and
   ...
   agent; the RC51 V4 agent must not be used to reinterpret it. Do not use the
...
- Existing V3 pair: do not recapture or reinterpret it. With the exact RC24
  ...
  generation only; it is not authorization to start the RC51 receiver.
- First installation with no pair: before applying 0011, stop one upstream
  ...
  RC51 source-agent image to capture that source once and immediately run
  ...
  and restore-test the full backup in explicit RC51 pre-0011 mode while the
...
1. Keep the verified pre-0011 package. Create, sign and restore-test a separate
   ...
   image recorded for that recovery generation, never RC51.
2. Stop all source agents. Install the RC51 v4 semantic-fingerprint functions
...
INVOICE_TOOLS_IMAGE='<exact RC51 tools image>' \
...
to the timestamp of the last payment is an RC51 rollback condition.
