...
1. verify signed tag `v0.1.0-rc17-signed` peels to commit
   ...
   manifest's exact rollback image IDs; verify the exact RC23 candidate images,
...
- Existing pair: do not recapture it. With the exact RC23 source-agent image,
...
- First installation with no pair: before applying 0011, stop one upstream
  ...
  RC23 source-agent image to capture that source once and immediately run
  ...
  and restore-test the full backup in explicit RC23 pre-0011 mode while the
...
INVOICE_TOOLS_IMAGE='<exact RC23 tools image>' \
SOURCE_AGENT_IMAGE='<exact RC23 source-agent image>' \
