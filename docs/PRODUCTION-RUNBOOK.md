...
1. verify signed tag `v0.1.0-rc17-signed` peels to commit
   ...
   manifest's exact rollback image IDs; verify the exact RC21 candidate images,
...
- Existing pair: do not recapture it. With the exact RC21 source-agent image,
...
- First installation with no pair: before applying 0011, stop one upstream
  ...
  RC21 source-agent image to capture that source once and immediately run
  ...
  and restore-test the full backup in explicit RC21 pre-0011 mode while the
...
INVOICE_TOOLS_IMAGE='<exact RC21 tools image>' \
SOURCE_AGENT_IMAGE='<exact RC21 source-agent image>' \
