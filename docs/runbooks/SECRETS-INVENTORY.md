...
| `secret://console-assertion/signing-key-<版本>` | 控制台断言签名私钥,见第五节 |
...
| `xm_console_assertion_signing_2026-09b.seed` | 控制台断言签名私钥的种子(base64),见第五节 | 只在粘进平台凭据库时用一次 |
...
**本次为什么不按规格的「valid_from 设在未来、重叠 ≥1 天」**:那是给计划内轮换
用的,为的是覆盖两侧发布的时间差。这次是私钥出现在截图里的应急轮换,多等一天
就是多暴露一天,所以新钥即刻生效,不锁门改由上面的顺序保证。
