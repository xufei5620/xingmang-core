...
// Keycloak's values to the console assertion's, so the console's first
// assertion login (auth.ResolveOrCreate, keyed exclusively on that pair) lands
// on the same historical row instead of minting an orphaned second identity.
