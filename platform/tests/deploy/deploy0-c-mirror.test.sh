...
expect_success "默认 github remote 执行一次镜像" env REAL_GIT="$real_git" D0C_TRACE="$trace" XM_DEPLOY_TEST_MODE=1 \
  "$mirror_script" --test-mode --repo "$success_repo" --server-url "$success_origin" --git-bin "$fake_git" --reason accepted
assert_text "镜像输出包含经过校验的 reason" 'reason=accepted' "$tmp/stdout"
...
assert_not_text "不修改 origin" "set-url origin" "$trace"
assert_not_text "不向 origin push" "push origin" "$trace"
...
expect_success "dry-run 不执行 push" env REAL_GIT="$real_git" D0C_TRACE="$dry_trace" XM_DEPLOY_TEST_MODE=1 \
  "$mirror_script" --test-mode --repo "$dry_repo" --server-url "$dry_origin" --git-bin "$fake_git" --dry-run
...
expect_failure "push 失败时返回非零" env REAL_GIT="$real_git" D0C_TRACE="$fail_trace" D0C_PUSH_FAIL=1 XM_DEPLOY_TEST_MODE=1 \
  "$mirror_script" --test-mode --repo "$fail_repo" --server-url "$fail_origin" --git-bin "$fake_git" --reason failed
assert_text "镜像失败输出可追溯摘要" 'evidence_sha256=' "$tmp/stderr"
...
expect_failure "含凭据的 remote URL 被拒绝" env REAL_GIT="$real_git" D0C_TRACE="$secret_trace" XM_DEPLOY_TEST_MODE=1 \
  "$mirror_script" --test-mode --repo "$secret_repo" --server-url "$secret_origin" --git-bin "$fake_git" --dry-run
