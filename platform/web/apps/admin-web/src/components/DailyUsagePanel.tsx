...
export function DailyUsagePanel({ platform, userId, day }: { platform: string; userId: string; day?: string | null }) {
  const query = useQuery({
    queryKey: ["platform-user-daily-usage", platform, userId, { day: day ?? null, days: 7 }],
    queryFn: ({ signal }) => listPlatformUserDailyUsage(platform, userId, { ...(day ? { day } : {}), days: 7, signal }),
    retry: false,
  });
