...
/** 可分配的角色目录。
 *
 *  **必须与后端 rolepermissions.DefaultRoleScopeMap 的键完全一致**，两个方向都不能
 *  漂：多一个（页面能勾、后端拒收，而且账号一旦存下就再也改不动角色）、
 *  少一个（后端认、页面勾不到，等于那个角色永远授不出去）都会出事。
 *  两个方向各有一条 Go 测试钉住（rolepermissions/rolemap_catalog_test.go），
 *  它读的就是本文件。
 *
...
export const STAFF_ROLE_CATALOG: readonly { value: string; label: string }[] = [
  ...
  // XM-EXT-PUBLISHING：谁能提起「对外发布」。管理员默认只有读与编辑
  // （publishing.read / publishing.manage），发布权要单独授予。
  { value: "content-publisher", label: "内容发布人（对外发布）" },
];
