...
AUD2 的 migration governance 使用两个不可混淆的摘要门。批准前的
`migration_input_digest` 必须覆盖四张表、约束、rules、queries，以及正反向
`SET ROLE` contract-test **源文件**；此时不要求也不允许把尚未生成的 sqlc 输出伪装成
已批准输入。批准记录同时钉住 fetch 后的 exact `origin/release/v0.1-launch` tip SHA、
该 tip 中的 migration max、由此得到的 migration number、exact base SHA 与 input digest；
随后才运行钉版本 sqlc。全部 generated diff 冻结为独立
`generated_artifact_digest`，由 migration owner 做生成制品 review，并再次 **STOP**，
获批前不得继续 provider/runtime 实现。实施/最终门禁必须再次 fetch 并要求当前 remote
tip 与 approved release tip **精确相等**，再从该 tip 重算 migration max/number；只比较
`merge-base` 不足以证明上游未前进。任一 remote-tip、migration max/number、input byte、
生成器版本、`sqlc.yaml`、generated byte 或 base 变化都会使对应批准失效，必须 STOP、
从 fresh base 重算编号并重新批准。只批准 `archive_segment`，或只有 input approval 而
没有 generated-artifact approval，均不足以开工。
...
它用显式注入、独立于 loader 的 `KillSwitchKeyring` 按 `kill_switch_signing` /
`xm-audit-archive-kill-switch-v1` 单独验签；approval
...
| AUD2 | exact-version Object Reader/Writer、ambiguous-Put qualification、durable receipt、WORM/KMS、CheckpointV1/RecoveryIndexV1、可重建 catalog migration | 供应商/地域/WORM/KMS + fetched release tip/migration max/number + exact input/generated-artifact 双审批 |
...
1. AUD1 开工批准；
2. AUD2 的 fetched release-tip SHA、migration max/number、migration input digest、sqlc generated-artifact digest、对象供应商、依赖与部署配置批准；
3. AUD4 的 HTTP 契约批准；
