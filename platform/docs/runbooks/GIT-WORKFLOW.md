# Runbook：服务器中心 Git 工作流（XM-C-DEPLOY0-c，历史过渡协议）

2026-09-14 复审：第 1–5 节描述的临时协议已于 2026-09-12 到期，原命令保留供
明确选择旧链兼容任务时参考，不再作为新任务的默认步骤。当前源码与服务器边界见
[Monorepo 迁移与发布链边界](../../../docs/MONOREPO-MIGRATION.md)，交付、合并和发布
按本轮明确授权及宪法第三、五节执行。此标记不延长旧例外、不自动恢复 Actions/PR，
也不授权更换 remote、运行整库镜像或改动生产。第 6 节的独立测试库技术说明保留。

历史背景：当时 GitHub Actions 停摆，服务器裸仓库承载开发、门禁和发布闭环，
GitHub 只保存镜像，不承担放行判断。
