...
> 状态：**DONE（十二项全部实现，2026-09-05）**。计划与实现均由本会话完成。
> 保留原文供审读与回溯；每一项的落地情况见第 5 节。
...
十二项全部完成，分四个提交，每个都过了全部门禁
（后端 `go test -p 1 ./...` 含真实 Postgres 集成测试、`go vet`、
`check-governance.sh`；前端 admin-web 1501 + ui-admin 261 用例、两个包 `tsc`）。

| 项 | 结果 |
|---|---|
| P1 开卡当场拉卡面 | 卡已 active 就立刻 reveal 落库；失败不影响开卡（周期同步兜底）；未 active 不拉 |
| P2 开卡费落库 | `issue_fee_text` / `issue_pay_amount_text`；同步传空保留原值（集成测试守着）；页面「开卡费」列 |
| P3 两账号余额 | `GET /api/v1/cards/balances`，实时上游调用不落投影；单账号失败只标记那一个；页面顶部余额条 |
| P4 批量状态 | 每账号每 100 张一次 `BatchCardStatus`，只对状态变化的卡再单查；250 张从 250 次降到 3 次 |
| P5 流水三字段 | 外币原值 + 结算时间往返；`settled_at` 为空存 NULL、页面显示「授权中」 |
| P6 CVV/有效期列 | 默认隐藏，随 `card.reveal` 门控；有效期原样显示不解析 |
| P7 关停卡 | 连接器 `DeleteCard`（请求字段 `card_id`）、Action `cards.card.delete`、页面两步确认；**显式排除天然幂等** |
| P8 回调交易语义 | `transaction_id` 等四字段落库；金额仍不从载荷写 |
| P9 3DS 验证挑战 | 新表 + 端点 + 徽章；验证码按 `card.reveal` 门控；**降级条款用上了**——真实事件没有验证码字段，无码时只显示「待验证」 |
| P10 状态枚举 | 补 `pending_delete → 删除中` |
| P11 错误码文案 | 业务码按处置分档；Handler 把分类翻成指向下一步的中文，「可能已生效」的两类明确劝阻重试 |
| P12 交易文案 | 类型/状态中文化，查表前统一小写（上游两处大小写不同） |

### 实现期的三个发现

**一、`card.challenge` 的真实载荷没有验证码。** 官方文档示例带
`"challenge":"123456"`，2026-09-05 生产实际收到的事件只有 `challenge_id` /
`challenge_type` / `expires_at`。计划里为此留的降级条款用上了：验证码是可选的，
无码时只提示「待验证」。按文档假定它一定存在，会做出一个永远空白的栏位。

**二、关停不能复用冻结的幂等豁免。** 冻结/解冻重复执行不产生新效果，所以
`naturallyIdempotent` 让它们的超时落 `failed`（可重试）。关停不可逆且会触发
余额结清，必须落 `unknown` 交给对账——已在 `naturallyIdempotent` 里显式排除，
并有测试钉住。

**三、卡 API 确实没有错误码表。** 四份 OpenAPI 里卡端点只定义了 200 响应，
错误码页也只有收单/提现/付款三节。所以 P11 只对确实有据的通用码与资金码分档，
卡端点的业务失败落到 `rejected` 默认分支——按猜测编一张卡端点错误码表，
只会在猜错时把一次「可能已生效」说成「确定没生效」。

### 新增迁移

`000030`（开卡费 + 流水三列）、`000031`（回调事件的交易身份）、
`000032`（3DS 挑战表）。部署时一并应用。
