...
## XM-CARD6：提现与流水同步（2026-09-05）

两件事一起做：把钱从 Infini 账号转出去（提现），以及把流水周期同步的默认
从「关」翻成「开」。

### 一、提现：唯一一个把钱转出平台的动作

链上转账没有撤回，这句话不是修辞。设计上的每一处取舍都从它推出来。

**权限不给 admin。** 卡片的四个权限当初全给了 admin，理由是「不给就等于
功能对唯一能用它的人 403」。提现不适用同一条理由——它不可逆，而 admin 是
日常操作账号；一个被盗用的 admin 会话不该能把资金池搬空。提现由新增的
`fund-operator` 角色持有（`fund.withdraw` + `fund.address.manage`），要人
显式授予。这与 `assurance-probe-admin` 是同一条设计意图，只是重一档。

**风险等级仍是 L1，并且这一点必须被读到。** 内核对 L2 及以上返回
`ADVANCED_CONTROLS_REQUIRED`（Foundation-B 未实现），声明成 L2 会让提现变成
永远跑不起来的摆设。**这意味着平台今天没有第二人审批这道闸**——L1 在这里
不是「评估过风险不高」，而是「内核只支持这一档」。契约 JSON 的 notes 里
写明了，免得以后有人把它读成前者。

实际的护栏有六道：独立权限、地址白名单、额度、幂等键、审计、两步确认。

**地址白名单。** 提现表单不能手打地址，只能从已登记的条目里选，且 Action
只收 `address_id`，地址由服务端从库里取。传地址进来再比对是另一回事——那样
一个比对逻辑的疏漏就能让任意地址过去。白名单挡不住有人拿密钥直接打 Infini
（那条路平台管不了），但能挡住误操作与通过后台的滥用，而这两样才是日常真正
会发生的。链跟着地址走、不让人另选一遍：两处各选一次就有了不一致的可能，
而转错链的钱找不回来。

**额度与卡片额度是两套。** 这一处差点写成 bug。生产上卡片额度按裁定配成
`unlimited`（「只要 infini 那边有余额就可以开」，余额本身是硬顶）；提现的
目的恰恰是把余额搬空，余额不构成任何约束，所以 `CheckWithdraw` 拒绝
`unlimited`。若两者共用一个字段，提现会永远抛 `ErrWithdrawLimitsUnbounded`
——一个能启动、一按就报错的功能，而这种「fail-closed 到不可用」要到上线
当天才会被发现。测试 `TestWithdrawLimitsAreSeparateFromCardLimits` 钉住它。

**额度存在库里，在管理后台改**（产品负责人 2026-09-05 决定，同日改掉了
最初的环境变量方案）。理由是他给的：一个要登服务器改文件再重启才能动的
数字，实际上没人会去动——它会永远停在第一次拍脑袋定的那个值上，然后在
真正要用的时候挡住正事。而重启 API 还会把正在用后台的人踢下线。

我最初的顾虑是「能被使用者自己调高的上限不是上限」。这条顾虑没有消失，
但它指向的解法不是「让它改不了」，而是**两把钥匙**：

    fund.limit.manage（admin）        —— 能抬高天花板，不能提现
    fund.withdraw    （fund-operator）—— 能提现，抬不高天花板

拿到任一把都搬不空资金池。今天同一个人两个角色都持有，这个分离在实践上
是名义的；但审计里「抬高上限」与「发起提现」是两条独立记录，而且想拆给
两个人时拆得开——并成一个权限就再也拆不开了。额度写入前会用一笔最小金额
跑一遍 `CheckWithdraw`（复用提现真正用的那条校验，而不是另写一套：另写
必然漂开，而漂开的方向永远是「存进去时说没问题、真用时被拒」）。

**没有行 = 该账号不能提现。** 这是新装环境的默认，也是唯一正确的默认：
一个「没人设过上限」的账号若能提现，等于上限的默认值是无穷大。页面上显示
成「未设置——该账号目前不能提现」而不是 0：后者是有人刻意关掉了它，
两者的下一步动作完全不同。每次提现都现读额度，不做进程内缓存——缓存会让
「刚调完额度却还是被拒」变成一个要等几分钟才自愈的怪现象。

**幂等。** 上游对 `request_id` 有真幂等（重复会回 `is_duplicate` 而不是转
两次），但同键重投时我们连上游都不打：少一次调用就少一次出错的机会。页面上
幂等键在**第一次点击（上膛）时**生成，两步之间不变；上游失败后按钮**停在
已上膛态**，因为回到未上膛会重新生成一个键，而那时人看到的还是同一笔提现。

**校验顺序全部在打上游之前**：账号解析 → 白名单 → 账号归属 → 链一致性 →
额度 → 幂等台账 → 上游。拦晚了钱就已经出去了。

**今日累计把未收敛的也算进去。** pending 的那笔可能真的转出去了；当它没转过
会让单日上限在最需要生效的时刻失效——恰恰是上游正在慢、人在反复点的时候。
只有确定失败（上游明确拒绝）的不算。

**审计摘要不带完整地址**（只有链、标签与 `address_id`）。地址本身不是秘密，
但审计摘要会被广泛展示，而一个转账地址被人抄走去做钓鱼是真事。要查转到哪儿
去看台账，那里有登记时的地址快照——登记条目以后可能被改名或删除，而「这笔钱
当时转到哪儿」必须永远查得到。

**读端点由 `fund.withdraw` 把守，不是 `card.read`。** 地址清单回的是完整
转账地址，历史回的是逐笔资金流向；两者都不该因为「能看卡」就顺带能看。
历史里给出链上哈希与区块浏览器链接——「平台说转成功了」和「链上确实有这笔」
是两件事，后者才是钱到没到的依据。

### 二、流水同步默认开启

原先默认关闭，代码里写的理由是「调用量与卡数成正比，而上游限流阈值未知」。
那个未知已经没有了：实测 600 次/分钟/密钥。按 250 张卡、5 分钟一轮是
50 次/分钟（8%），而卡状态刷新同期改成了批量（250 张卡从 250 次调用降到
3 次），恰好把原先被它占掉的预算让了出来。

回调（XM-CARD4）已能秒级推进单卡流水，所以这一轮周期同步是**兜底**：补回调
丢掉的、以及回调里没有的历史。兜底默认开着才有意义。无法识别的值按开启处理，
因为这个开关的两个方向不对称——多同步一轮的代价是几十次调用，少同步的代价是
流水长期缺失而没有任何报错。只有显式的 `false` 才关闭。

### 上线前要做的两件事

1. **授予 `fund-operator` 角色**给该有提现权限的人。不授予的话提现区会是
   403，而不是「按钮不见了」——按钮凭空消失是最难查的症状，所以 Action 的
   注册不按配置决定有无。
2. **配提现额度**：管理端「卡片管理 → 提现 → 改额度」，真数字，不能是
   「不限」。**不再是环境变量**，不用登服务器。不配则该账号不能提现。

### 已知的空缺

- **不校验地址格式。** 那需要按链各实现一套，而校验不全比不校验更容易让人
  放松——一个"通过了校验"的错地址比一个没校验过的更容易被点下去。页面上写
  明了让人自己核对首尾几位。
- **没有第二人审批**，理由见上（L1 天花板）。
- **提现记录不能删。** 台账是 append-only（与操作台账同一条纪律），页面上
  也没有删除按钮——一笔转出去的钱不该能从记录里消失。

### 地址的下线与改指（2026-09-05 补）

地址可以在后台**停用**（`cards.withdraw.address.set_enabled`），不是删除。
一条曾经被列入白名单的地址，它存在过这件事本身就是审计事实——「这条地址
当初是谁登记的、什么时候下线的」正是出事之后第一个要问的问题，删掉就再也
回答不了。已发出的提现不受影响：台账里存的是登记时的地址快照。

停用的地址留在清单里（标记「已停用」，可重新启用），但提现表单不提供它，
**领域层也在打上游之前拒绝它**——只靠前端不渲染是不够的，一个还留着旧页面
的标签页仍然能提交。错误文案区分「未登记」与「已停用」：前者去登记，后者
去启用。为此把哨兵错误的措辞改成中性的「提现地址不可用」，四种情形
（未登记 / 已停用 / 账号不符 / 链不符）各自在包起它时把话说清。

**地址登记 id 不能被改指到另一条地址。** 这条不变量原本只由主键隐式保证，
报出来是一句裸的 Postgres 约束错；现在翻成人话了。它要紧的原因是：改指意味着
以后所有选中「冷钱包」的提现都悄悄转去了新地方，而页面上标签一个字都没变
——那正是攻击者会做的事，而且从界面上完全看不出来。要换地址只能新登记一条
再把旧的停用。反过来，重新登记一条**同样的**地址会把它启用，否则运营会在
一条停用的地址上反复点「登记」，每次都显示成功却仍然提不了现。

### 一处测试自查

「表单只提供启用的地址」最初断言的是下拉框里没有那一项。Select 是 Radix 的，
选项在 Portal 里、只有展开才存在，所以**把过滤代码删掉那条断言照样通过**
（实测过）。换成了真正可感知的形态：地址全部停用时提现表单整个不出现，
删掉过滤会精确地让它变红。


---

## XM-CARD9：交易记录、订阅与字段对齐（2026-09-05）

产品负责人：「跟 infini 那边的字段对齐，这样使用上不会不清楚」。他每天在两个
界面之间切换，同一个东西两个名字是纯粹的认知税。
...
后两项上游 OpenAPI 里没有对应字段（`contracts/connectors/infini/openapi/card.yaml`
里查不到 limit / low_balance / threshold），画一个出来只能是编的。

交易记录表的列名逐字照抄：时间 / 商户 / 交易类型 / 结算金额 / 交易金额 /
卡片名称 / 手续费 / 状态。

### 交易记录：新增跨卡读法

此前只有「按卡查流水」。跨卡视图需要一个新端点 `GET /cards/transactions`，
**卡片名称在 SQL 里 join**：一张跨卡流水表最要紧的定位信息就是「这笔是哪张卡
刷的」，让前端拿 card_id 再去卡片列表里配对，等于把一次 join 挪到浏览器里做
——而那张列表可能因为筛选或分页根本没加载全，配不上的行只能显示裸 id。

LEFT JOIN 而不是 INNER：卡关停后投影行会消失，但**流水必须还在**（钱确实
花了）。join 不上时名称为空，前端退回卡号后四位。

汇总磁贴只做算得出来的三个（本月消费 / 本月手续费 / 本月笔数）。Infini 的
「本月预估返现」「累计返现」不做——上游没有返现的读法，编一个数出来会被
当成钱。本月消费只统计**已完成**的：授权中的金额还会变，算进去会让这个数
随后自己缩水。

### 订阅：同一个页签，不同的数据来源

Infini 那一页是从交易记录**自动识别**的，他们自己在页面上标着「仅供参考，
可能与实际订阅不一致」。我们不猜：只列运营在「用途登记」里填过的。

代价是没登记的不出现；好处是出现的每一条都可信。**错了的续费提醒比没有提醒
更糟**——人会拿它做预算、会信它。试用转正、年付转月付、涨价、首月折扣、汇率
波动，任何一样都会让推断悄悄错掉，而错在哪儿没有任何迹象。

为此给用途登记加了两个字段（迁移 000036）：`subscription_amount_text` 与
`subscription_cycle`。周期的取值约束放在 Action Schema 而不是数据库枚举——
上游的订阅五花八门（双月、季付、按量），写死数据库枚举只会让第一个不在表里
的周期没法登记，而 Schema 改起来不需要迁移。

「每月订阅支出」只把周期为 monthly 的加起来。把年付摊成月付看着更完整，但
那需要一个记账口径上的假设；宁可少算也不给一个说不清怎么来的数。页面上会
显式说明「其中 N 条没登记金额，未计入合计」——一个偏低的合计比没有合计更
容易误导预算。

### 本地预览的做法（可复用）

`XM_AUTH_MODE=dev-header` + scratchpad 里的一份临时 vite 配置（给代理注入
`X-Dev-Principal-ID/Type/Scopes`）。**scopes 头是逗号分隔的**，写成空格分隔会
一路 403 而错误信息只说「缺少权限 card.read」。配置放 scratchpad 不进仓库：
它注入的开发身份头是生产绝不该有的东西；依赖用 `file:///` 绝对路径导入，
因为那个位置不在任何 node_modules 的解析链上。
