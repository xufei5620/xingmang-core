...
1. **AI 身份不放开。** 消费者是平台里的功能通过接口接入，那正是 `SERVICE`
   这一类；AI 会话自己要号不在计划内。三个消费者动作维持只放 `SERVICE`。
2. **不为机器身份做任何登录侧的改动。** 本切片只**预留接口**：Action + 契约 +
   配额骨架 + 规范文档。平台是模块化单体，以后要用接码的功能就在这个进程里，
   直接调领域服务或走同一套 Action 即可，不需要另一套身份。
   平台外的独立服务要打进来才需要「非人」身份，而现有登录系统（Keycloak）后续
   要弃用——在一个要被换掉的东西上搭机器身份，换的时候要拆两遍。等新的登录体系
   落定再按那时的样子接。
   （2026-09-06 一度按 Keycloak 服务账号实现过一版并写了 CR-0010，产品负责人指出
   Keycloak 将弃用后已整体撤掉，未推远端。）
