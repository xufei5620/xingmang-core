...
4. 原迁移和权限作业、新栈闩通过后，按已审 SHA 原子安装候选 vhost；`nginx -T` 通过，再执行 `nginx -s reload`。
5. 对配置的真实 origin 运行只读冒烟。切换或 reload/冒烟失败进入原自动回滚。

回滚先停新写者、恢复原权限和完整旧 22 容器并核原 ready，再原子恢复快照中的 vhost，语法检查、reload，实际读取两个域名的 `/readyz`。只有这些完成才记录 `ROLLED_BACK`。已损坏或丢失的候选 vhost 不妨碍使用原快照恢复；不覆盖切换之外产生的未知 live 文件改动。切换中断后用匹配配置和持久快照运行原 rollback 入口。
