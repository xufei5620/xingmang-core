...
from role_policy import explicit_policy, validate_resolved


def main():
    parser=argparse.ArgumentParser()
    for name in ['platform-service','invoice-service','role-map','admin-role','mode','staff-origin','crosswalk','output-directory','psql']:
    ...
    try:
        ...
        if args.mode not in ['server-rehearsal','local-synthetic']:raise ValueError('INVALID_CAPTURE_MODE')
        admin_role,roles=explicit_policy(args.admin_role,role_map(read_json(args.role_map)));mapping=read_json(args.crosswalk)
        ...
        for name,service,sqlfile in [('platform',args.platform_service,'staff-mfa.sql'),('invoice',args.invoice_service,'invoice-actors.sql')]:
            ...
            if name=='platform':command+=['-v','role_scope_map_json='+json.dumps(roles,separators=(',',':')),'-v','admin_role='+admin_role]
        ...
        mfa={'schema':'xingmang.identity-audit.mfa/v2','mode':args.mode,'status':'FAIL',
             'role':'finance.read','total':result['finance_read_total'],
             'totp_registered':result['finance_read_totp_registered'],
             'role_policy':result['role_policy'],'end_utc':records[0]['endUtc'],
             'platform_snapshot_sha256':result['input_sha256']['platform'],
             'platform_snapshot':str((output/'platform.json').absolute())}
        try:
            validate_resolved({'ADMIN_ROLE':admin_role,'XM_AUTH_ROLE_SCOPES':json.dumps(roles)},mfa['role_policy'])
            if mfa['total']>0 and mfa['totp_registered']==mfa['total']:mfa['status']='PASS'
        except ValueError:pass
        (output/'mfa-query-record.json').write_text(json.dumps(mfa,indent=2))
        (output/'capture-execution.json').write_text(json.dumps(records,indent=2))
        print('Read-only capture complete; result.json contains metadata and explicit unresolved conditions.')
        return 2 if result['blockers'] or mfa['status']!='PASS' else 0
