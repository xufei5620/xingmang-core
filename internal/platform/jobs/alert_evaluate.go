...
func (w *AlertEvaluateWorker) Work(ctx context.Context, job *river.Job[AlertEvaluateArgs]) error {
	...
	w.logJob(ctx, job, level, "job_completed", res.NotifyFailed == 0, errorCode,
		...
		// 四个「这一轮为什么与你想的不一样」的计数必须逐轮可见：一个不留痕的
		// 抑制器就是下一个「安静地给你一个旧答案」。suppressed＝本轮失败但迟滞
		// 门槛没到、held＝本轮已恢复但迟滞还没放手、version_ack＝版本变过但有人
		// 核对过、chronic_held＝最新一轮采集已成功但 R4 的窗口计数还没回落。
		...
		slog.Int("chronic_held", res.ChronicHeld),
