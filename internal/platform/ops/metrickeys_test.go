...
// TestRegisteredMetricsMatchSub2APIContract 是白名单与契约之间的防漂移闸门。
//
// ops 里的白名单是**字面量重复**：connectors/sub2api 依赖 ops
// （ToObservations 返回 ops.Observation），反向 import 会成环，所以 ops 不能
// 直接引用那五个 Metric* 常量。重复的代价由本用例兜住——它在外部测试包里，
// 同时看得见两边，任何一边加减指标都会当场失败。
//
...
// 更难查。
func TestRegisteredMetricsMatchSub2APIContract(t *testing.T) {
	fromContract := []string{
	...
	if len(registered) != len(fromContract) {
		t.Fatalf("白名单 = %v（%d 条），契约 = %v（%d 条）；两边必须逐条对齐",
			registered, len(registered), fromContract, len(fromContract))
	}
}
