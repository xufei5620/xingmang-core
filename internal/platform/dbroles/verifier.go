...
// PostgreSQL's PUBLIC ACL principal is represented by grantee OID 0. It is
// not a role name accepted by has_*_privilege, so inspect the ACL directly
// instead of passing the literal pseudo-role to those helpers.
const catalogPublicDatabasePrivilegesSQL = `SELECT ax.privilege_type FROM pg_database AS d CROSS JOIN LATERAL aclexplode(COALESCE(d.datacl, acldefault('d'::"char", d.datdba))) AS ax WHERE d.datname = current_database() AND ax.grantee = 0 ORDER BY ax.privilege_type`
const catalogPublicSchemaPrivilegesSQL = `SELECT ax.privilege_type FROM pg_namespace AS n CROSS JOIN LATERAL aclexplode(COALESCE(n.nspacl, acldefault('n'::"char", n.nspowner))) AS ax WHERE n.nspname = 'public' AND ax.grantee = 0 ORDER BY ax.privilege_type`
const catalogSchemasSQL = `SELECT n.nspname, owner.rolname, grantees.rolname, privileges.privilege_type FROM pg_namespace AS n JOIN pg_roles AS owner ON owner.oid = n.nspowner CROSS JOIN (VALUES ('xm_migrator'), ('xm_api_runtime'), ('xm_worker_runtime'), ('xm_lifecycle_runtime'), ('xm_ops_read'), ('xm_backup_read')) AS grantees(rolname) CROSS JOIN (VALUES ('USAGE'), ('CREATE')) AS privileges(privilege_type) WHERE n.nspname IN ('public','core','action','audit','ops','alerts','finance','ui') AND has_schema_privilege(grantees.rolname, n.nspname, privileges.privilege_type) UNION ALL SELECT n.nspname, owner.rolname, 'PUBLIC', ax.privilege_type FROM pg_namespace AS n JOIN pg_roles AS owner ON owner.oid = n.nspowner CROSS JOIN LATERAL aclexplode(COALESCE(n.nspacl, acldefault('n'::"char", n.nspowner))) AS ax WHERE n.nspname IN ('public','core','action','audit','ops','alerts','finance','ui') AND ax.grantee = 0 ORDER BY 1, 3, 4`
const catalogDatabaseACLsSQL = `SELECT d.datname, owner.rolname, grantees.rolname, privileges.privilege_type FROM pg_database AS d JOIN pg_roles AS owner ON owner.oid = d.datdba CROSS JOIN (VALUES ('xm_migrator'), ('xm_api_runtime'), ('xm_worker_runtime'), ('xm_lifecycle_runtime'), ('xm_ops_read'), ('xm_backup_read')) AS grantees(rolname) CROSS JOIN (VALUES ('CONNECT'), ('CREATE'), ('TEMPORARY')) AS privileges(privilege_type) WHERE d.datname = current_database() AND has_database_privilege(grantees.rolname, d.datname, privileges.privilege_type) UNION ALL SELECT d.datname, owner.rolname, 'PUBLIC', ax.privilege_type FROM pg_database AS d JOIN pg_roles AS owner ON owner.oid = d.datdba CROSS JOIN LATERAL aclexplode(COALESCE(d.datacl, acldefault('d'::"char", d.datdba))) AS ax WHERE d.datname = current_database() AND ax.grantee = 0 ORDER BY 3, 4`
