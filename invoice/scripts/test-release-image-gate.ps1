...
foreach ($validStrictDirectory in @('release\0.1.0-mono-rehearsal1-exact1', 'release\0.1.0-mono-rehearsal1-exact99')) {
    Assert-StrictReleaseDirectoryName -ReleaseDirectory $validStrictDirectory -ExpectedReleaseName '0.1.0-mono-rehearsal1' | Out-Null
}
foreach ($invalidStrictDirectory in @(
    'release\0.1.0-rc52-exact1',
    'release\0.1.0-MONO-REHEARSAL1-exact1',
    'release\0.1.0-mono-rehearsal1-exact0',
    'release\0.1.0-mono-rehearsal1-exact100',
    'release\rc100-exact1'
)) {
    Assert-ThrowsLike `
        -Action { Assert-StrictReleaseDirectoryName -ReleaseDirectory $invalidStrictDirectory -ExpectedReleaseName '0.1.0-mono-rehearsal1' | Out-Null } `
...
$validStrictManifestJson = '{"releaseName":"0.1.0-mono-rehearsal1","images":[{"name":"api","reference":"invoice-system-api:0.1.0-mono-rehearsal1"}]}'
Assert-JsonHasNoDuplicateProperties -JsonText $validStrictManifestJson | Out-Null
foreach ($duplicateJsonFixture in @(
    [pscustomobject]@{ Label = 'same-case top-level releaseName'; Json = '{"releaseName":"0.1.0-rc52","releaseName":"0.1.0-mono-rehearsal1"}' },
    [pscustomobject]@{ Label = 'case-drifted top-level releaseName'; Json = '{"releaseName":"0.1.0-rc52","ReleaseName":"0.1.0-mono-rehearsal1"}' },
    [pscustomobject]@{ Label = 'same-case nested image reference'; Json = '{"images":[{"reference":"invoice-system-api:0.1.0-rc52","reference":"invoice-system-api:0.1.0-mono-rehearsal1"}]}' },
    [pscustomobject]@{ Label = 'case-drifted nested image reference'; Json = '{"images":[{"reference":"invoice-system-api:0.1.0-rc52","Reference":"invoice-system-api:0.1.0-mono-rehearsal1"}]}' }
)) {
    Assert-ThrowsLike `
...
if (-not $artifactVerifierSource.Contains("Assert-StrictReleaseDirectoryName -ReleaseDirectory `$releaseRoot -ExpectedReleaseName '0.1.0-mono-rehearsal1'", [StringComparison]::Ordinal)) {
...
Test-Task5ARequirement -Label 'exact RC100 tag name maps to the fully qualified tag ref' -Action {
    $resolvedTagRef = Get-StrictSignedReleaseTagRef -SignedReleaseTag 'rehearsal/mono-20260910-signed'
    if ($resolvedTagRef -cne 'refs/tags/rehearsal/mono-20260910-signed') {
...
foreach ($invalidTagName in @(
    ...
    'v0.1.0-rc51-signed',
    'v0.1.0-rc52-signed',
    'rehearsal/mono-20260910-signed-sibling',
    'refs/heads/rehearsal/mono-20260910-signed',
    'refs/tags/rehearsal/mono-20260910-signed'
)) {
...
Test-Task5ARequirement -Label 'production runbook invokes the exact RC100 strict transfer-ready verifier parameters' -Action {
    if (-not $productionRunbook.Contains('-RequireTransferReady', [StringComparison]::Ordinal) -or
        -not $productionRunbook.Contains('-SignedReleaseTag rehearsal/mono-20260910-signed', [StringComparison]::Ordinal) -or
...
$transferReadyManifest = [pscustomobject]@{
    releaseName = '0.1.0-mono-rehearsal1'
    source = [pscustomobject]@{ gitDirty = $false; gitHead = '0123456789abcdef0123456789abcdef01234567' }
    images = @(
        [pscustomobject]@{ name = 'api'; reference = 'invoice-system-api:0.1.0-mono-rehearsal1' },
        [pscustomobject]@{ name = 'pdf-scanner'; reference = 'invoice-system-pdf-scanner:0.1.0-mono-rehearsal1' },
        [pscustomobject]@{ name = 'tools'; reference = 'invoice-system-tools:0.1.0-mono-rehearsal1' },
        [pscustomobject]@{ name = 'web'; reference = 'invoice-system-web:0.1.0-mono-rehearsal1' },
        [pscustomobject]@{ name = 'source-agent'; reference = 'invoice-source-agent:0.1.0-mono-rehearsal1' },
        [pscustomobject]@{ name = 'postgres-runtime'; reference = 'invoice-postgres:0.1.0-mono-rehearsal1' },
        [pscustomobject]@{ name = 'clamav-runtime'; reference = 'invoice-clamav:0.1.0-mono-rehearsal1' },
        [pscustomobject]@{ name = 'ingest-proxy'; reference = 'invoice-ingest-proxy:0.1.0-mono-rehearsal1' },
        [pscustomobject]@{ name = 'keycloak'; reference = 'invoice-keycloak:0.1.0-mono-rehearsal1' }
    )
...
Assert-TransferManifestMutationRejected -Mutate { param($manifest) $manifest.releaseName = '0.1.0-rc52' } -ExpectedMessagePattern 'releaseName=0\.1\.0-mono-rehearsal1' -FailureMessage 'strict transfer mode accepted the failed RC52 release name'
Assert-TransferManifestMutationRejected -Mutate { param($manifest) $manifest.releaseName = '0.1.0-mono-rehearsal1-sibling' } -ExpectedMessagePattern 'releaseName=0\.1\.0-mono-rehearsal1' -FailureMessage 'strict transfer mode accepted a sibling RC100 release name'
Assert-TransferManifestMutationRejected -Mutate { param($manifest) $manifest.releaseName = '0.1.0-MONO-REHEARSAL1' } -ExpectedMessagePattern 'releaseName=0\.1\.0-mono-rehearsal1' -FailureMessage 'strict transfer mode accepted a case-drifted RC100 release name'
foreach ($invalidReleaseNameFixture in @(
    [pscustomobject]@{ Label = 'null'; Value = $null },
    [pscustomobject]@{ Label = 'Boolean'; Value = $false },
    [pscustomobject]@{ Label = 'singleton array'; Value = [string[]]@('0.1.0-mono-rehearsal1') }
)) {
    Assert-TransferManifestMutationRejected `
        -Mutate { param($manifest) $manifest.releaseName = $invalidReleaseNameFixture.Value } `
        -ExpectedMessagePattern 'releaseName=0\.1\.0-mono-rehearsal1' `
...
Assert-TransferManifestMutationRejected `
    -Mutate {
        ...
        $manifest | Add-Member -NotePropertyName 'ReleaseName' -NotePropertyValue '0.1.0-mono-rehearsal1'
    } `
...
Assert-TransferManifestMutationRejected `
    -Mutate {
        ...
        $manifest.images[0] | Add-Member -NotePropertyName 'Reference' -NotePropertyValue 'invoice-system-api:0.1.0-mono-rehearsal1'
    } `
...
foreach ($inventoryMutation in @(
    ...
    [pscustomobject]@{
        ...
        Mutate = { param($manifest) $manifest.images = 'invoice-system-api:0.1.0-mono-rehearsal1' }
    },
    [pscustomobject]@{
    ...
    [pscustomobject]@{
        ...
        Mutate = {
            ...
            $manifest.images = @($manifest.images) + [pscustomobject]@{
                name = 'unexpected'
                reference = 'invoice-unexpected:0.1.0-mono-rehearsal1'
            }
        }
    },
    [pscustomobject]@{
    ...
    [pscustomobject]@{
        ...
        Expected = 'exact RC100 image inventory'
        Mutate = { param($manifest) $manifest.images[0].reference = 'Invoice-system-api:0.1.0-mono-rehearsal1' }
    }
)) {
    Assert-TransferManifestMutationRejected `
...
foreach ($imageStringField in @('name', 'reference')) {
    $validSingletonValue = if ($imageStringField -ceq 'name') { 'api' } else { 'invoice-system-api:0.1.0-mono-rehearsal1' }
...
foreach ($expectedImage in @($transferReadyManifest.images)) {
    foreach ($wrongTag in @('0.1.0-rc52', '0.1.0-mono-rehearsal1-sibling')) {
        Assert-TransferManifestMutationRejected `
            -Mutate {
                param($manifest)
                $record = @($manifest.images | Where-Object { [string]$_.name -ceq [string]$expectedImage.name })
                $record[0].reference = ([string]$record[0].reference) -replace ':0\.1\.0-mono-rehearsal1$', ":$wrongTag"
            } `
            -ExpectedMessagePattern 'exact RC100 image inventory' `
