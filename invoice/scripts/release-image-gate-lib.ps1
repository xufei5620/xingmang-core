...
function Assert-PostgresGosuFindingScope {
    ...
    foreach ($finding in @($Summary.Findings)) {
        ...
        if ($matchingTuple.Count -ne 1) {
            throw "PostgreSQL exception has no approved RC100 no-fix tuple for $($finding.Target):$($finding.VulnerabilityID)"
        }
    }
    return $true
}
...
function Get-StrictSignedReleaseTagRef {
    ...
    if ($SignedReleaseTag -cne 'rehearsal/mono-20260910-r2-signed') {
        throw 'strict transfer requires the exact signed RC100 tag rehearsal/mono-20260910-r2-signed'
    }
    return 'refs/tags/rehearsal/mono-20260910-r2-signed'
}
...
function Assert-TransferReadyManifest {
    ...
    try {
        ...
        throw 'strict transfer requires exact property releaseName; manifest releaseName=0.1.0-mono-rehearsal2 is mandatory'
    }
    if ($releaseNameProperty.Value -isnot [string] -or
        [string]$releaseNameProperty.Value -cne '0.1.0-mono-rehearsal2') {
        throw 'strict transfer requires manifest releaseName=0.1.0-mono-rehearsal2'
    }

    $expectedImageReferences = [ordered]@{
        api = 'invoice-system-api:0.1.0-mono-rehearsal2'
        'pdf-scanner' = 'invoice-system-pdf-scanner:0.1.0-mono-rehearsal2'
        tools = 'invoice-system-tools:0.1.0-mono-rehearsal2'
        web = 'invoice-system-web:0.1.0-mono-rehearsal2'
        'source-agent' = 'invoice-source-agent:0.1.0-mono-rehearsal2'
        'postgres-runtime' = 'invoice-postgres:0.1.0-mono-rehearsal2'
        'clamav-runtime' = 'invoice-clamav:0.1.0-mono-rehearsal2'
        'ingest-proxy' = 'invoice-ingest-proxy:0.1.0-mono-rehearsal2'
        keycloak = 'invoice-keycloak:0.1.0-mono-rehearsal2'
    }
    ...
    if ($imagesProperty.Value -isnot [System.Array] -or
        ...
        throw 'strict transfer requires the exact RC100 image inventory'
    }
