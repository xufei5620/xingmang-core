...
// current trusted staff resolver's tuple, so it resolves the same historical
// row instead of minting a second identity. This offline maintenance tool
// does not restore an OIDC login or assertion exchange. See
// deploy/unified/audit/IDENTITY-MIGRATE.md for the read-only check and pending
// server work. Defaults to dry-run (omit --apply); --apply requires an approving
