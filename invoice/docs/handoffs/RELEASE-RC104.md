...
- 发布签名身份已核（**未触碰私钥内容**）：用 `~/.ssh/invoice_release_allowed_signers`
  ...
  **手册第 438 行那条 `cmd /c ... < file` 写法在本仓库的 `发票` 中文路径段上会报
  ...
  签的是 tag 与 `SHA256SUMS`，**提交本身从不签名**——本版实测 RC100–RC103 四个
  签名 tag 指向的提交 `%G?` 全是 `N`，而手册镜像门禁那段却写着
  `git verify-commit HEAD` 并带 `throw`。这条落差由 RC105 的 `d28bb8b` 写进手册
  第 3 节「照抄本节会撞上的五处」的第 ① 条。
  **2026-09-09 已按「改文档」定案**：`git verify-commit HEAD` 及其 `throw` 从手册
  删除，判据换成「`git verify-tag` Good 且签名 tag 剥离后的提交逐字等于 HEAD」，
  理由（发布签名密钥只在发布时由主控者用一次）写在手册那段代码块之后。
