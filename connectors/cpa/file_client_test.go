...
// happens to use, and that a completely unresolved role degrades to an
// honestly-reported 0 rather than a query error.
...
`
	...
	if !health.Healthy {
		t.Fatalf("Health = %+v, want Healthy=true (missing cache columns degrade gracefully, they do not break the connection)", health)
	}
	if health.Detail == "" {
