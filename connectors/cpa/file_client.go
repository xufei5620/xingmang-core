...
func (c *fileClient) Health(ctx context.Context) (connector.HealthResult, error) {
	...
	if len(missing) > 0 {
		detail = fmt.Sprintf(
			"token 列未按候选名单解析: %s（对应用量/成本按 0 计，非崩溃；见 contracts/connectors/cpa.read.v1.md §8）",
...
func (c *fileClient) UsageSummary(ctx context.Context, day string) (UsageSummary, error) {
	...
	cols, _, err := resolveTokenColumns(ctx, db)
	if err != nil {
		return UsageSummary{}, connector.NewError(connector.KindBadResponse, op, err)
	}
	...
	return UsageSummary{
		Snapshot: Snapshot{
			...
			IsPartial: unpricedRequests > 0, Instance: FileInstance,
		},
		BusinessDay:          day,
		Rows:                 usageRows,
...
func (c *fileClient) KeyUsage(ctx context.Context, day string) (KeyUsagePage, error) {
	...
	return KeyUsagePage{
		Snapshot: Snapshot{
			ObservedAt: observed, Watermark: observed.Format(time.RFC3339),
			IsPartial: isPartial, Instance: FileInstance,
		},
		BusinessDay:   day,
		Rows:          out,
