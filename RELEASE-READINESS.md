# Release readiness: RC32 historical issuer lifecycle and RC60 image-security candidate

Status as of 2026-08-31:

- prior application/full non-image baseline at `75fac62522a823ce8854f11f6bd4fa8ff82b526e`:
  **GO** (`scripts/verify.ps1` passed); the current RC60 script/document delta
  has only targeted verification and must rerun the full gate before signing;
- isolated staging deployment: **GO only after the fresh RC60 image/SBOM gate
...
## RC60 source/static security stage (current)

**Production: NO-GO.**  RC60 has source/static changes only.  The complete
release evidence is still absent: no RC60 Docker build, zero-finding scan, SBOM,
...
reuse, rename, amend, or transfer exact1 as RC60 evidence.
...
tag or resume, rewrite, rename, reuse, or transfer exact1 as RC60 evidence.
...
rewrite, rename, reuse, or transfer exact1 as RC60 evidence.
...
derived ingest proxy, and Keycloak.  PostgreSQL is zero-finding with no RC60
...
RC60 keeps the two-stage hard gate: first create and verify the signed source
...
- generate and independently verify a fresh RC60 image/SBOM/vulnerability
...
- deploy all Invoice images under the same RC60 tag and pass real production
...
- create and locally verify the RC60 commit and signed release tag; no GitHub
