...
foreach ($validStrictDirectory in @('release\0.1.0-rc94-exact1', 'release\0.1.0-rc94-exact99')) {
    Assert-StrictReleaseDirectoryName -ReleaseDirectory $validStrictDirectory -ExpectedReleaseName '0.1.0-rc94' | Out-Null
}
foreach ($invalidStrictDirectory in @(
    'release\0.1.0-rc52-exact1',
    'release\0.1.0-RC94-exact1',
    'release\0.1.0-rc94-exact0',
    'release\0.1.0-rc94-exact100',
    'release\rc94-exact1'
)) {
    Assert-ThrowsLike `
        -Action { Assert-StrictReleaseDirectoryName -ReleaseDirectory $invalidStrictDirectory -ExpectedReleaseName '0.1.0-rc94' | Out-Null } `
...
$validStrictManifestJson = '{"releaseName":"0.1.0-rc94","images":[{"name":"api","reference":"invoice-system-api:0.1.0-rc94"}]}'
Assert-JsonHasNoDuplicateProperties -JsonText $validStrictManifestJson | Out-Null
foreach ($duplicateJsonFixture in @(
    [pscustomobject]@{ Label = 'same-case top-level releaseName'; Json = '{"releaseName":"0.1.0-rc52","releaseName":"0.1.0-rc94"}' },
    [pscustomobject]@{ Label = 'case-drifted top-level releaseName'; Json = '{"releaseName":"0.1.0-rc52","ReleaseName":"0.1.0-rc94"}' },
    [pscustomobject]@{ Label = 'same-case nested image reference'; Json = '{"images":[{"reference":"invoice-system-api:0.1.0-rc52","reference":"invoice-system-api:0.1.0-rc94"}]}' },
    [pscustomobject]@{ Label = 'case-drifted nested image reference'; Json = '{"images":[{"reference":"invoice-system-api:0.1.0-rc52","Reference":"invoice-system-api:0.1.0-rc94"}]}' }
)) {
    Assert-ThrowsLike `
...
if (-not (Test-Path -LiteralPath $postgresDockerfilePath) -or
    ...
    throw 'RC94 derived PostgreSQL, ClamAV, and ingest runtime image sources are missing'
}
...
foreach ($requiredTask2Helper in @(
    ...
    if (-not (Get-Command -Name $requiredTask2Helper -ErrorAction SilentlyContinue)) {
        throw "RC94 focused static checker is missing: $requiredTask2Helper"
    }
}
...
if ($apiBaseStart -lt 0 -or $scannerBaseStart -le $apiBaseStart) {
    throw 'RC94 backend fixture does not contain the named runtime stages'
}
...
if (-not $webDockerfile.Contains($fixedAlpinePackages) -or
    ...
    throw 'RC94 runtime Alpine stages do not pin both fixed OpenSSL packages'
}
if (-not $postgresDockerfile.Contains($goBuilderReference) -or
    ...
    throw 'RC94 PostgreSQL gosu rebuild is not pinned, platform-gated, and read-only at build time'
}
Assert-ExactNormalizedText -Actual $postgresGoMod -Expected $expectedGosuGoMod -Label 'RC94 PostgreSQL gosu go.mod'
Assert-ExactNormalizedText -Actual $postgresGoSum -Expected $expectedGosuGoSum -Label 'RC94 PostgreSQL gosu go.sum'
if (-not $clamavDockerfile.Contains($clamavBaseReference) -or
    ...
    throw 'RC94 PostgreSQL, ClamAV, or ingest runtime is not pinned to the reviewed base'
}
...
if ($productionComposeText -match '(?m)^\s+image:\s+postgres:18-alpine@sha256:d3e1620b530c944afa6e887d22eb899824da68e19c52024bf98f5220c88a65b2\s*$' -or
    ...
    throw 'RC94 production Compose retains external PostgreSQL, ClamAV, or ingest runtime image references'
}
...
Assert-ThrowsLike `
    ...
    -ExpectedMessagePattern 'no approved RC94 no-fix tuple' `
...
if ($artifactVerifierSource -match 'approved-by-exact-binary-exception' -or
    ...
    throw 'RC94 artifact verification still permits a PostgreSQL vulnerability exception'
}
...
if (-not $artifactVerifierSource.Contains("Assert-StrictReleaseDirectoryName -ReleaseDirectory `$releaseRoot -ExpectedReleaseName '0.1.0-rc94'", [StringComparison]::Ordinal)) {
    throw 'independent artifact verifier does not bind strict transfer to an RC94 exactN directory leaf'
}
...
Test-Task5ARequirement -Label 'exact RC94 tag name maps to the fully qualified tag ref' -Action {
    $resolvedTagRef = Get-StrictSignedReleaseTagRef -SignedReleaseTag 'v0.1.0-rc94-signed'
    if ($resolvedTagRef -cne 'refs/tags/v0.1.0-rc94-signed') {
...
foreach ($invalidTagName in @(
    ...
    'v0.1.0-rc51-signed',
    'v0.1.0-rc52-signed',
    'v0.1.0-rc94-signed-sibling',
    'refs/heads/v0.1.0-rc94-signed',
    'refs/tags/v0.1.0-rc94-signed'
)) {
...
Test-Task5ARequirement -Label 'production runbook invokes the exact RC94 strict transfer-ready verifier parameters' -Action {
    if (-not $productionRunbook.Contains('-RequireTransferReady', [StringComparison]::Ordinal) -or
        -not $productionRunbook.Contains('-SignedReleaseTag v0.1.0-rc94-signed', [StringComparison]::Ordinal) -or
...
function Assert-RC94ProductionPrerequisites {
    ...
    foreach ($requiredRC94Instruction in @(
        'fresh RC94 image/SBOM/vulnerability',
        'same RC94 tag',
        'RC94 commit and signed release tag'
    )) {
        if (-not $prerequisites.Contains($requiredRC94Instruction, [StringComparison]::Ordinal)) {
            throw "release readiness production prerequisites are missing RC94 instruction: $requiredRC94Instruction"
        }
    }
...
Assert-RC94ProductionPrerequisites -ReadinessText $releaseReadiness
foreach ($staleInstructionMutation in @(
    [pscustomobject]@{ Current = 'fresh RC94 image/SBOM/vulnerability'; Stale = 'fresh RC52 image/SBOM/vulnerability' },
    [pscustomobject]@{ Current = 'same RC94 tag'; Stale = 'same RC52 tag' },
    [pscustomobject]@{ Current = 'RC94 commit and signed release tag'; Stale = 'RC52 commit and signed release tag' }
)) {
    Test-Task5AMutationRejected -Label "stale readiness instruction $($staleInstructionMutation.Stale)" -Action {
        Assert-RC94ProductionPrerequisites -ReadinessText ($releaseReadiness.Replace(
...
function Assert-RC94ImageScanReviewCurrentSection {
    param([Parameter(Mandatory)][string]$ReviewText)

    $sectionStart = $ReviewText.IndexOf('## Current RC94 remediation and review status (source/static only)', [StringComparison]::Ordinal)
    $sectionEnd = $ReviewText.IndexOf('## Historical RC1 approved-candidate snapshot (retired)', [StringComparison]::Ordinal)
    if ($sectionStart -lt 0 -or $sectionEnd -le $sectionStart) {
        throw 'image scan review does not contain a bounded current RC94 section'
    }
    $currentReview = $ReviewText.Substring($sectionStart, $sectionEnd - $sectionStart)
    foreach ($requiredRC94SecurityContract in @(
        'PostgreSQL has **no RC94 exception**',
        'The sole permitted RC94 exception',
        ...
        if (-not $currentReview.Contains($requiredRC94SecurityContract, [StringComparison]::Ordinal)) {
            throw "current RC94 image scan review is missing exact security contract: $requiredRC94SecurityContract"
        }
    }
    foreach ($retiredSecurityContract in @(
        ...
        if ($currentReview.Contains($retiredSecurityContract, [StringComparison]::Ordinal)) {
            throw "current RC94 image scan review includes retired security contract: $retiredSecurityContract"
        }
    }
}

$imageScanReview = Get-Content -Raw -LiteralPath (Join-Path $projectRoot 'docs\IMAGE-SCAN-REVIEW.md')
Assert-RC94ImageScanReviewCurrentSection -ReviewText $imageScanReview
Test-Task5AMutationRejected -Label 'retired Keycloak base injected into current RC94 scan section' -Action {
    Assert-RC94ImageScanReviewCurrentSection -ReviewText ($imageScanReview.Replace(
        'sha256:9d1f1b2b7261ff53c66cb1092dfcdc34a5fb77e81f9e6a6e75b8b6a795de8067',
        'sha256:6efbadc00f0ed0237610becf11f4101b9c3ad8edf08a5b70c97aa4154ed436ec'
    ))
}
...
foreach ($supersededRC49Document in @(
    ...
    if (-not $documentPrefix.Contains('**SUPERSEDED — DO NOT EXECUTE.**', [StringComparison]::Ordinal) -or
        -not $documentPrefix.Contains('docs/superpowers/plans/2026-09-05-invoice-rc94-release-recovery.md', [StringComparison]::Ordinal)) {
...
$rc94RecoveryPlan = Get-Content -Raw -LiteralPath (Join-Path $projectRoot 'docs\superpowers\plans\2026-09-05-invoice-rc94-release-recovery.md')
if (-not $failureEvidenceVerifierSource.Contains('Assert-RC49FailureEvidenceAnchor -ProjectRoot $projectRoot -AnchorText $anchorText', [StringComparison]::Ordinal) -or
    ...
    -not $rc94RecoveryPlan.Contains('all four failure-evidence scripts', [StringComparison]::Ordinal) -or
    ...
    throw 'RC94 recovery flow does not invoke all four dedicated failure evidence anchor verifiers'
}
foreach ($requiredRC94RecoveryContract in @(
    'PowerShell 7.5+',
    'exit `42`',
    ...
    if (-not $rc94RecoveryPlan.Contains($requiredRC94RecoveryContract, [StringComparison]::Ordinal)) {
        throw "RC94 recovery plan is missing fail-closed contract: $requiredRC94RecoveryContract"
    }
}
if ($rc94RecoveryPlan -match '\[string\]\$rc94Manifest') {
    throw 'RC94 recovery plan manually coerces manifest decisions instead of trusting the verifiers'
}
...
function Assert-RC94DerivedImageGateSource {
    param(
    ...
    foreach ($requiredDefinition in @(
        ...
        if (-not $Gate.Contains($requiredDefinition, [StringComparison]::Ordinal)) {
            throw "RC94 release gate is missing locally built derived image definition: $requiredDefinition"
        }
    }
    foreach ($baseBinding in @(
        ...
        if (-not $Gate.Contains($baseBinding, [StringComparison]::Ordinal)) {
            throw "RC94 derived image definition is missing exact base binding: $baseBinding"
        }
    }
    foreach ($contextBinding in @(
        ...
        if ($Gate -notmatch "(?m)^\s+$([regex]::Escape($contextBinding.Fingerprint)) = Get-ContextFingerprint" -or
            ...
            throw "RC94 release gate does not fingerprint and build exact context $($contextBinding.Context)"
        }
    }
    if ($Gate -notmatch "@\('build', '--pull', '--platform', 'linux/amd64', '--provenance=false'" -or
        ...
        throw 'RC94 locally built images are not build-time and post-build gated to linux/amd64'
    }
    if ($Gate -match "exact-postgres-gosu-exception|approved-by-exact-binary-exception|postgres_exception_proof_failed" -or
        ...
        throw 'RC94 release gate retains an active PostgreSQL exception/proof path'
    }
    ...
    if (-not $ArtifactVerifier.Contains("Assert-LinuxAmd64Platform -Platform ([string]`$record.platform)", [StringComparison]::Ordinal) -or
        ...
        throw 'independent artifact verifier does not reject non-built or non-linux/amd64 RC94 image records'
    }
}
...
Assert-RC94DerivedImageGateSource -Gate $gateSource -Library $gateLibrarySource -ArtifactVerifier $artifactVerifierSource
...
$bindingFixtureRoot = Join-Path ([IO.Path]::GetTempPath()) ('invoice-rc94-artifact-binding-' + [guid]::NewGuid().ToString('N'))
...
$transferReadyManifest = [pscustomobject]@{
    releaseName = '0.1.0-rc94'
    source = [pscustomobject]@{ gitDirty = $false; gitHead = '0123456789abcdef0123456789abcdef01234567' }
    images = @(
        [pscustomobject]@{ name = 'api'; reference = 'invoice-system-api:0.1.0-rc94' },
        [pscustomobject]@{ name = 'pdf-scanner'; reference = 'invoice-system-pdf-scanner:0.1.0-rc94' },
        [pscustomobject]@{ name = 'tools'; reference = 'invoice-system-tools:0.1.0-rc94' },
        [pscustomobject]@{ name = 'web'; reference = 'invoice-system-web:0.1.0-rc94' },
        [pscustomobject]@{ name = 'source-agent'; reference = 'invoice-source-agent:0.1.0-rc94' },
        [pscustomobject]@{ name = 'postgres-runtime'; reference = 'invoice-postgres:0.1.0-rc94' },
        [pscustomobject]@{ name = 'clamav-runtime'; reference = 'invoice-clamav:0.1.0-rc94' },
        [pscustomobject]@{ name = 'ingest-proxy'; reference = 'invoice-ingest-proxy:0.1.0-rc94' },
        [pscustomobject]@{ name = 'keycloak'; reference = 'invoice-keycloak:0.1.0-rc94' }
    )
...
Assert-TransferManifestMutationRejected -Mutate { param($manifest) $manifest.releaseName = '0.1.0-rc52' } -ExpectedMessagePattern 'releaseName=0\.1\.0-rc94' -FailureMessage 'strict transfer mode accepted the failed RC52 release name'
Assert-TransferManifestMutationRejected -Mutate { param($manifest) $manifest.releaseName = '0.1.0-rc94-sibling' } -ExpectedMessagePattern 'releaseName=0\.1\.0-rc94' -FailureMessage 'strict transfer mode accepted a sibling RC94 release name'
Assert-TransferManifestMutationRejected -Mutate { param($manifest) $manifest.releaseName = '0.1.0-RC94' } -ExpectedMessagePattern 'releaseName=0\.1\.0-rc94' -FailureMessage 'strict transfer mode accepted a case-drifted RC94 release name'
foreach ($invalidReleaseNameFixture in @(
    [pscustomobject]@{ Label = 'null'; Value = $null },
    [pscustomobject]@{ Label = 'Boolean'; Value = $false },
    [pscustomobject]@{ Label = 'singleton array'; Value = [string[]]@('0.1.0-rc94') }
)) {
    Assert-TransferManifestMutationRejected `
        -Mutate { param($manifest) $manifest.releaseName = $invalidReleaseNameFixture.Value } `
        -ExpectedMessagePattern 'releaseName=0\.1\.0-rc94' `
...
Assert-TransferManifestMutationRejected `
    -Mutate {
        ...
        $manifest | Add-Member -NotePropertyName 'ReleaseName' -NotePropertyValue '0.1.0-rc94'
    } `
...
Assert-TransferManifestMutationRejected `
    -Mutate {
        ...
        $manifest.images[0] | Add-Member -NotePropertyName 'Reference' -NotePropertyValue 'invoice-system-api:0.1.0-rc94'
    } `
...
foreach ($inventoryMutation in @(
    ...
    [pscustomobject]@{
        ...
        Expected = 'exact RC94 image inventory'
        Mutate = { param($manifest) $manifest.images = 'invoice-system-api:0.1.0-rc94' }
    },
    [pscustomobject]@{
    ...
    [pscustomobject]@{
        ...
        Mutate = {
            ...
            $manifest.images = @($manifest.images) + [pscustomobject]@{
                name = 'unexpected'
                reference = 'invoice-unexpected:0.1.0-rc94'
            }
        }
    },
    [pscustomobject]@{
    ...
    [pscustomobject]@{
        ...
        Expected = 'exact RC94 image inventory'
        Mutate = { param($manifest) $manifest.images[0].reference = 'Invoice-system-api:0.1.0-rc94' }
    }
)) {
    Assert-TransferManifestMutationRejected `
...
foreach ($imageStringField in @('name', 'reference')) {
    $validSingletonValue = if ($imageStringField -ceq 'name') { 'api' } else { 'invoice-system-api:0.1.0-rc94' }
    foreach ($invalidImageStringFixture in @(
        ...
        Assert-TransferManifestMutationRejected `
            ...
            -ExpectedMessagePattern 'exact RC94 image inventory' `
...
foreach ($expectedImage in @($transferReadyManifest.images)) {
    foreach ($wrongTag in @('0.1.0-rc52', '0.1.0-rc94-sibling')) {
        Assert-TransferManifestMutationRejected `
            -Mutate {
                param($manifest)
                $record = @($manifest.images | Where-Object { [string]$_.name -ceq [string]$expectedImage.name })
                $record[0].reference = ([string]$record[0].reference) -replace ':0\.1\.0-rc94$', ":$wrongTag"
            } `
            -ExpectedMessagePattern 'exact RC94 image inventory' `
...
function Assert-RC94RuntimeAndBackupBindings {
    ...
    foreach ($binding in @(
        ...
        if (-not $binding.Source.Contains($binding.Required, [StringComparison]::Ordinal)) {
            throw "RC94 verifier is missing expected ID binding: $($binding.Label)"
        }
    }
    ...
    if ($backup -match 'nginx:1\.30-alpine@sha256' -or $restore -match 'postgres:18(?:\.6)?-alpine@sha256' -or
        ...
        throw 'current backup/restore drill is not bound to existing no-pull RC94 derived images'
    }
}

Assert-RC94RuntimeAndBackupBindings -ProjectRoot $projectRoot
