...
function Assert-PostgresGosuFindingScope {
    ...
    foreach ($finding in @($Summary.Findings)) {
        ...
        if ($matchingTuple.Count -ne 1) {
            throw "PostgreSQL exception has no approved RC66 no-fix tuple for $($finding.Target):$($finding.VulnerabilityID)"
        }
    }
    return $true
}
...
function Get-StrictSignedReleaseTagRef {
    ...
    if ($SignedReleaseTag -cne 'v0.1.0-rc66-signed') {
        throw 'strict transfer requires the exact signed RC66 tag v0.1.0-rc66-signed'
    }
    return 'refs/tags/v0.1.0-rc66-signed'
}
...
function Assert-TransferReadyManifest {
    ...
    try {
        ...
        throw 'strict transfer requires exact property releaseName; manifest releaseName=0.1.0-rc66 is mandatory'
    }
    if ($releaseNameProperty.Value -isnot [string] -or
        [string]$releaseNameProperty.Value -cne '0.1.0-rc66') {
        throw 'strict transfer requires manifest releaseName=0.1.0-rc66'
    }

    $expectedImageReferences = [ordered]@{
        api = 'invoice-system-api:0.1.0-rc66'
        'pdf-scanner' = 'invoice-system-pdf-scanner:0.1.0-rc66'
        tools = 'invoice-system-tools:0.1.0-rc66'
        web = 'invoice-system-web:0.1.0-rc66'
        'source-agent' = 'invoice-source-agent:0.1.0-rc66'
        'postgres-runtime' = 'invoice-postgres:0.1.0-rc66'
        'clamav-runtime' = 'invoice-clamav:0.1.0-rc66'
        'ingest-proxy' = 'invoice-ingest-proxy:0.1.0-rc66'
        keycloak = 'invoice-keycloak:0.1.0-rc66'
    }
    ...
    if ($imagesProperty.Value -isnot [System.Array] -or
        ...
        throw 'strict transfer requires the exact RC66 image inventory'
    }
