...
function Assert-PostgresGosuFindingScope {
    ...
    foreach ($finding in @($Summary.Findings)) {
        ...
        if ($matchingTuple.Count -ne 1) {
            throw "PostgreSQL exception has no approved RC88 no-fix tuple for $($finding.Target):$($finding.VulnerabilityID)"
        }
    }
    return $true
}
...
function Get-StrictSignedReleaseTagRef {
    ...
    if ($SignedReleaseTag -cne 'v0.1.0-rc88-signed') {
        throw 'strict transfer requires the exact signed RC88 tag v0.1.0-rc88-signed'
    }
    return 'refs/tags/v0.1.0-rc88-signed'
}
...
function Assert-TransferReadyManifest {
    ...
    try {
        ...
        throw 'strict transfer requires exact property releaseName; manifest releaseName=0.1.0-rc88 is mandatory'
    }
    if ($releaseNameProperty.Value -isnot [string] -or
        [string]$releaseNameProperty.Value -cne '0.1.0-rc88') {
        throw 'strict transfer requires manifest releaseName=0.1.0-rc88'
    }

    $expectedImageReferences = [ordered]@{
        api = 'invoice-system-api:0.1.0-rc88'
        'pdf-scanner' = 'invoice-system-pdf-scanner:0.1.0-rc88'
        tools = 'invoice-system-tools:0.1.0-rc88'
        web = 'invoice-system-web:0.1.0-rc88'
        'source-agent' = 'invoice-source-agent:0.1.0-rc88'
        'postgres-runtime' = 'invoice-postgres:0.1.0-rc88'
        'clamav-runtime' = 'invoice-clamav:0.1.0-rc88'
        'ingest-proxy' = 'invoice-ingest-proxy:0.1.0-rc88'
        keycloak = 'invoice-keycloak:0.1.0-rc88'
    }
    ...
    if ($imagesProperty.Value -isnot [System.Array] -or
        ...
        throw 'strict transfer requires the exact RC88 image inventory'
    }
