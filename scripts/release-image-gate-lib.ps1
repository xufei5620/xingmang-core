...
function Assert-PostgresGosuFindingScope {
    ...
    foreach ($finding in @($Summary.Findings)) {
        ...
        if ($matchingTuple.Count -ne 1) {
            throw "PostgreSQL exception has no approved RC87 no-fix tuple for $($finding.Target):$($finding.VulnerabilityID)"
        }
    }
    return $true
}
...
function Get-StrictSignedReleaseTagRef {
    ...
    if ($SignedReleaseTag -cne 'v0.1.0-rc87-signed') {
        throw 'strict transfer requires the exact signed RC87 tag v0.1.0-rc87-signed'
    }
    return 'refs/tags/v0.1.0-rc87-signed'
}
...
function Assert-TransferReadyManifest {
    ...
    try {
        ...
        throw 'strict transfer requires exact property releaseName; manifest releaseName=0.1.0-rc87 is mandatory'
    }
    if ($releaseNameProperty.Value -isnot [string] -or
        [string]$releaseNameProperty.Value -cne '0.1.0-rc87') {
        throw 'strict transfer requires manifest releaseName=0.1.0-rc87'
    }

    $expectedImageReferences = [ordered]@{
        api = 'invoice-system-api:0.1.0-rc87'
        'pdf-scanner' = 'invoice-system-pdf-scanner:0.1.0-rc87'
        tools = 'invoice-system-tools:0.1.0-rc87'
        web = 'invoice-system-web:0.1.0-rc87'
        'source-agent' = 'invoice-source-agent:0.1.0-rc87'
        'postgres-runtime' = 'invoice-postgres:0.1.0-rc87'
        'clamav-runtime' = 'invoice-clamav:0.1.0-rc87'
        'ingest-proxy' = 'invoice-ingest-proxy:0.1.0-rc87'
        keycloak = 'invoice-keycloak:0.1.0-rc87'
    }
    ...
    if ($imagesProperty.Value -isnot [System.Array] -or
        ...
        throw 'strict transfer requires the exact RC87 image inventory'
    }
