...
function Assert-PostgresGosuFindingScope {
    ...
    foreach ($finding in @($Summary.Findings)) {
        ...
        if ($matchingTuple.Count -ne 1) {
            throw "PostgreSQL exception has no approved RC81 no-fix tuple for $($finding.Target):$($finding.VulnerabilityID)"
        }
    }
    return $true
}
...
function Get-StrictSignedReleaseTagRef {
    ...
    if ($SignedReleaseTag -cne 'v0.1.0-rc81-signed') {
        throw 'strict transfer requires the exact signed RC81 tag v0.1.0-rc81-signed'
    }
    return 'refs/tags/v0.1.0-rc81-signed'
}
...
function Assert-TransferReadyManifest {
    ...
    try {
        ...
        throw 'strict transfer requires exact property releaseName; manifest releaseName=0.1.0-rc81 is mandatory'
    }
    if ($releaseNameProperty.Value -isnot [string] -or
        [string]$releaseNameProperty.Value -cne '0.1.0-rc81') {
        throw 'strict transfer requires manifest releaseName=0.1.0-rc81'
    }

    $expectedImageReferences = [ordered]@{
        api = 'invoice-system-api:0.1.0-rc81'
        'pdf-scanner' = 'invoice-system-pdf-scanner:0.1.0-rc81'
        tools = 'invoice-system-tools:0.1.0-rc81'
        web = 'invoice-system-web:0.1.0-rc81'
        'source-agent' = 'invoice-source-agent:0.1.0-rc81'
        'postgres-runtime' = 'invoice-postgres:0.1.0-rc81'
        'clamav-runtime' = 'invoice-clamav:0.1.0-rc81'
        'ingest-proxy' = 'invoice-ingest-proxy:0.1.0-rc81'
        keycloak = 'invoice-keycloak:0.1.0-rc81'
    }
    ...
    if ($imagesProperty.Value -isnot [System.Array] -or
        ...
        throw 'strict transfer requires the exact RC81 image inventory'
    }
